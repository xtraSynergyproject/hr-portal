...
namespace Synergy.App.Business
{
    public class EmailBusiness : BusinessBase<EmailViewModel, Email>, IEmailBusiness
    {
        ...
        public EmailBusiness(IRepositoryBase<EmailViewModel, Email> repo
            ...
            , IRepositoryQueryBase<MessageEmailViewModel> queryRepo
        ...
        private async Task SetEnvironment(MailMessage mailMessage, EmailViewModel email)
        {

            var appEnv = _configuration.GetValue<string>("ApplicationEnvironment");
            if (appEnv != "PROD" || !ValidateEmail(mailMessage))
            {
                var testRecipients = await _csb.GetSingle(x => x.Code == "TEST_EMAIL_RECIPIENTS");
                if (testRecipients != null)
                {
                    ...
                    var toList = testRecipients.Value.Split(';');
        ...
        private bool ValidateEmail(MailMessage mailMessage)
        {
        ...
        public async Task<CommandResult<EmailViewModel>> SendMail(EmailViewModel email)
        {
            ...
            try
            {
                await SetEnvironment(mailMessage, email);
        ...
        {
            if (email.DataAction != DataActionEnum.Edit)
            {
        ...
        private async Task<MailMessage> GetMailMessage(EmailViewModel email, EmailProperties config)
        {
            ...
            foreach (var item in toList)
            {
                if (item.IsNotNullAndNotEmpty() && item.Contains("@"))
                {
                    mailMessage.To.Add(item.Trim());
                }
            }
        ...
        private async Task<SmsViewModel> GetSmsConfig(string companyId)
        {
            var company = new CompanyViewModel();
            if (_repo.UserContext != null && _repo.UserContext.CompanyId.IsNotNullAndNotEmpty())
            {
            ...
            {
                company = await _companyBusiness.GetSingleById(companyId);
            }
            var ret = new SmsViewModel();
            if (company != null)
            {
                ret.SmsGateway = company.SmsGateway;
                ret.SmsUserId = company.SmsUserId;
                ret.SmsPassword = company.SmsPassword;
        ...
        public async Task<CommandResult<EmailViewModel>> SendMailAsync(EmailViewModel email)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    ...
                    await hangfireScheduler.Enqueue<HangfireScheduler>(x => x.SendMail(result.Item.Id, _repo.UserContext.ToIdentityUser()));
                }
                return CommandResult<EmailViewModel>.Instance(email, result.IsSuccess, result.Message);
            }
            catch (Exception ex)
        ...
        public async Task<EmailSetupViewModel> GetUserInobxEmailConfig(string userId)
        {
            ...
            if (details.IsNotNull())
            {
                ...
                var decryptPass = Helper.Decrypt(details.SmtpPassword);
                emailConfig = new EmailSetupViewModel()
                {
                    FromEmailId = details.SmtpFromId,
                    SenderName = details.UserName,
                    SmtpHost = details.SmtpHost,
        ...
        private async Task<MessageEmailViewModel> GetTaskMessageId(string MessageId)
        {
            ...
            var data = await _cmsQueryBusiness.GetTaskMessageIdData(MessageId);
            return data;
        }
        ...
        public async Task<CommandResult<EmailViewModel>> SendCalendarMail(EmailViewModel cEmail)
        {
            ...
            try
            {
                ...
                await SetEnvironment(mailMessage, cEmail);
        ...
        public async Task<CommandResult<SmsViewModel>> ManageSms(NotificationViewModel viewModel)
        {
            var appEnv = _configuration.GetValue<string>("ApplicationEnvironment");
            var allowSms = appEnv == "PROD" && viewModel.NotifyBySms && viewModel.Recipient != null &&
                viewModel.Recipient.Mobile.IsNotNullAndNotEmpty() && viewModel.SmsText.IsNotNullAndNotEmpty();

            if (allowSms)
            {
                var sms = await GetSmsConfig(viewModel.CompanyId);
                if (sms.SmsGateway.IsNotNullAndNotEmpty())
                {
                    sms.SendingType = SmsSendingTypeEnum.Individual;
                    sms.SmsText = viewModel.SmsText;
                    sms.Number = viewModel.Recipient.Mobile;
                    var url = sms.SmsGateway.Replace("{{TEXT}}", sms.SmsText).Replace("{{NUMBER}}", sms.Number);
                    try
                    {
                        sms.Response = await Helper.HttpRequest(url, HttpVerb.Get, null, true); ;
                        sms.SmsStatus = NotificationStatusEnum.Sent;
                    ...
                    {
                        await _fileBusiness.Create<SmsViewModel, Sms>(sms);
                    }
                }
                return CommandResult<SmsViewModel>.Instance(sms);
            }
            return CommandResult<SmsViewModel>.Instance(new SmsViewModel());
        }
    }
}
