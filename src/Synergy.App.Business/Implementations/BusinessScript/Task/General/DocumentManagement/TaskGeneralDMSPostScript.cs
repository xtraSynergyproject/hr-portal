using Synergy.App.Business.BusinessScript.Task.General.DocumentManagement;
...
namespace Synergy.App.Business.Implementations.BusinessScript.Task.General.DocumentManagement
{
    public class TaskGeneralDMSPostScript : ITaskGeneralDMSPostScript
    {
        ...
        public async Task<string> GetStageStatus(string templateCode, Dictionary<string, object> rowData, string currentStageStatus, IServiceProvider sp)
        {
            ...
            else if (templateCode == "PROJECT_DOC_TASK3" || templateCode == "GALFAR_VENDOR_STEP_TASK3" || templateCode == "ENGINEERING_SUBCONTRACT_STEP_TASK3" || templateCode == "INSPECTION_SER_TASK2_HALUL" || templateCode == "INSPECTION_SER_TASK2")
            {
                ...
                if (stageStatus.IsNotNull())
                {
                 ...
                 || templateCode == "DocReviewbyDisciplineEngg/Planning")
            {
        ...
        public async Task<CommandResult<TaskTemplateViewModel>> MoveDocumentDetail(TaskTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _noteBusiness = sp.GetService<INoteBusiness>();
            var _docBusiness = sp.GetService<IDMSDocumentBusiness>();
            var _lovBusiness = sp.GetService<ILOVBusiness>();
            ...
            if (viewModel.TaskStatusCode == "TASK_STATUS_COMPLETE" && documentId.IsNotNullAndNotEmpty())
            {
                var noteDetails = await _noteBusiness.GetSingleById(documentId);
                var noteTempModel = new NoteTemplateViewModel();
                noteTempModel.TemplateCode = noteDetails.TemplateCode;
                ...
                noteTempModel.SetUdfValue = true;
                var notemodel = await _noteBusiness.GetNoteDetails(noteTempModel);
                var parentFolder = notemodel.ColumnList.FirstOrDefault(x => x.Name == "ParentFolderId");
                var docCat = notemodel.ColumnList.FirstOrDefault(x => x.Name == "DocumentCategoryId");
                var revision = notemodel.ColumnList.FirstOrDefault(x => x.Name == "RevisionId");
                var parentFolderId = parentFolder.Value.ToString();
                var docCatId = docCat.Value.ToString();
                var revisionId = revision.Value.ToString();
                var lovParent = await _lovBusiness.GetSingleById(parentFolderId);
                var lovrevision = await _lovBusiness.GetSingleById(revisionId);
                var lovDocCat = await _lovBusiness.GetSingleById(docCatId);
                string workspace = await _docBusiness.GetWorkspaceByDocumentId(documentId);
                var noteParentFolderId = "";
                var noteDocCatId = "";
                var noteRev = "";
                var childfolderofworkspace = await _noteBusiness.GetSingle(x => x.ParentNoteId == workspace && x.NoteSubject == lovParent.Name);
                if (childfolderofworkspace.IsNotNull())
                {
                    noteParentFolderId = childfolderofworkspace.Id;
                }
                else
                {
                    ...
                    templateModel.TemplateCode = "GENERAL_FOLDER";
                    var newmodel = await _noteBusiness.GetNoteDetails(templateModel);
                    newmodel.NoteSubject = lovParent.Name;
                    newmodel.NoteStatusCode = "NOTE_STATUS_INPROGRESS";
                    newmodel.ParentNoteId = workspace;
                    // newmodel.SequenceOrder = ;
                    newmodel.OwnerUserId = uc.UserId;
                    dynamic exo = new System.Dynamic.ExpandoObject();
                    ((IDictionary<String, Object>)exo).Add("Code", lovParent.Name);
                    newmodel.Json = Newtonsoft.Json.JsonConvert.SerializeObject(exo);
                    var result = await _noteBusiness.ManageNote(newmodel);
                    if (result.IsSuccess)
                    {
                        noteParentFolderId = result.Item.NoteId;
                    }
                }
                if (noteParentFolderId.IsNotNullAndNotEmpty())
                {
                    var childfolderofParentFolder = await _noteBusiness.GetSingle(x => x.ParentNoteId == noteParentFolderId && x.NoteSubject == lovDocCat.Name);
                    if (childfolderofParentFolder.IsNotNull())
                    {
                        noteDocCatId = childfolderofParentFolder.Id;
                    }
                    else
                    {
                        ...
                        templateModel.TemplateCode = "GENERAL_FOLDER";
                        var newmodel = await _noteBusiness.GetNoteDetails(templateModel);
                        newmodel.NoteSubject = lovDocCat.Name;
                        newmodel.NoteStatusCode = "NOTE_STATUS_INPROGRESS";
                        newmodel.ParentNoteId = noteParentFolderId;
                        // newmodel.SequenceOrder = ;
                        newmodel.OwnerUserId = uc.UserId;
                        dynamic exo = new System.Dynamic.ExpandoObject();
                        ((IDictionary<String, Object>)exo).Add("Code", lovDocCat.Name);
                        newmodel.Json = Newtonsoft.Json.JsonConvert.SerializeObject(exo);
                        var result = await _noteBusiness.ManageNote(newmodel);
                        if (result.IsSuccess)
                        {
                            noteDocCatId = result.Item.NoteId;
                        }
                    }
                }
                if (noteDocCatId.IsNotNullAndNotEmpty())
                {
                    var childfolderofDoc = await _noteBusiness.GetSingle(x => x.ParentNoteId == noteDocCatId && x.NoteSubject == lovrevision.Name);
                    if (childfolderofDoc.IsNotNull())
                    {
                        noteRev = childfolderofDoc.Id;
                    }
                    else
                    {
                        ...
                        templateModel.TemplateCode = "GENERAL_FOLDER";
                        var newmodel = await _noteBusiness.GetNoteDetails(templateModel);
                        newmodel.NoteSubject = lovrevision.Name;
                        newmodel.NoteStatusCode = "NOTE_STATUS_INPROGRESS";
                        newmodel.ParentNoteId = noteDocCatId;
                        // newmodel.SequenceOrder = ;
                        newmodel.OwnerUserId = uc.UserId;
                        dynamic exo = new System.Dynamic.ExpandoObject();
                        ((IDictionary<String, Object>)exo).Add("Code", lovrevision.Name);
                        newmodel.Json = Newtonsoft.Json.JsonConvert.SerializeObject(exo);
                        var result = await _noteBusiness.ManageNote(newmodel);
                        if (result.IsSuccess)
                        {
                            noteRev = result.Item.NoteId;
                        }
                    }
                }
                if (noteRev.IsNotNullAndNotEmpty())
                {
                    var targetId = noteRev;
                    string sourceId = udf.documentId;
                    string previousParentId = noteDetails.ParentNoteId;
                    noteDetails.ParentNoteId = targetId;
                    IList<FolderViewModel> parents = await _docBusiness.GetAllParentByNoteId(sourceId);
                    var parentids = parents.Select(x => x.Id).ToList();
                    var parentnoteIds = string.Join(",", parentids);
                    var parentnotePermissions = await _docBusiness.GetAllNotePermissionByParentId(parentnoteIds);
                    var result = await _noteBusiness.Edit(noteDetails);
                    if (result.IsSuccess)
                    {
                        if (targetId.IsNotNull())
                        {
                            // Delete previous parent permission from note and its child
                            if (previousParentId.IsNotNullAndNotEmpty())
                            {
                        ...
                        if (tags.Count > 0)
                        {
                            ...
                            var res = await _docBusiness.UpdateTagsByDocumentIds(ids);
                            if (res)
                            {
                                var parentlist = await _docBusiness.GetAllParentByNoteId(sourceId);
        ...
        public async Task<CommandResult<TaskTemplateViewModel>> MoveDocumentStandardEngg(TaskTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _noteBusiness = sp.GetService<INoteBusiness>();
            var _docBusiness = sp.GetService<IDMSDocumentBusiness>();
            var _lovBusiness = sp.GetService<ILOVBusiness>();
            ...
            if (viewModel.TaskStatusCode == "TASK_STATUS_COMPLETE" && documentId.IsNotNullAndNotEmpty())
            {
                ...
                noteTempModel.SetUdfValue = true;
                var notemodel = await _noteBusiness.GetNoteDetails(noteTempModel);
                var parentFolder = notemodel.ColumnList.FirstOrDefault(x => x.Name == "DisciplineId");
                var docCat = notemodel.ColumnList.FirstOrDefault(x => x.Name == "DocumentCategoryId");
                var revision = notemodel.ColumnList.FirstOrDefault(x => x.Name == "RevisionId");
                var parentFolderId = parentFolder.Value.ToString();
                var docCatId = docCat.Value.ToString();
        ...
        public async Task<CommandResult<TaskTemplateViewModel>> MoveDocumentVendor(TaskTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _noteBusiness = sp.GetService<INoteBusiness>();
            var _docBusiness = sp.GetService<IDMSDocumentBusiness>();
            var _lovBusiness = sp.GetService<ILOVBusiness>();
        ...
        public async Task<CommandResult<TaskTemplateViewModel>> MoveDocumentProject(TaskTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _noteBusiness = sp.GetService<INoteBusiness>();
            var _docBusiness = sp.GetService<IDMSDocumentBusiness>();
            var _lovBusiness = sp.GetService<ILOVBusiness>();
            ...
            if (viewModel.TaskStatusCode == "TASK_STATUS_COMPLETE" && documentId.IsNotNullAndNotEmpty())
            {
                ...
                var docCat = notemodel.ColumnList.FirstOrDefault(x => x.Name == "DocumentCategoryId");
                var revision = notemodel.ColumnList.FirstOrDefault(x => x.Name == "RevisionId");
                var discipline = notemodel.ColumnList.FirstOrDefault(x => x.Name == "DisciplineId");
                var parentFolderId = parentFolder.Value.ToString();
                var docCatId = docCat.Value.ToString();
                var revisionId = revision.Value.ToString();
                var disciplineId = discipline.Value.ToString();
                var lovParent = await _lovBusiness.GetSingleById(parentFolderId);
                var lovrevision = await _lovBusiness.GetSingleById(revisionId);
                var lovDocCat = await _lovBusiness.GetSingleById(docCatId);
                var lovDiscipline = await _lovBusiness.GetSingleById(disciplineId);
                string workspace = await _docBusiness.GetWorkspaceByDocumentId(documentId);
                var noteParentFolderId = "";
                var noteDocCatId = "";
                var noteRev = "";
                ...
                if (noteParentFolderId.IsNotNullAndNotEmpty())
                {
                    var childfolderofParentFolder = await _noteBusiness.GetSingle(x => x.ParentNoteId == noteParentFolderId && x.NoteSubject == lovDiscipline.Name);
                    if (childfolderofParentFolder.IsNotNull())
                    {
                        noteDiscipline = childfolderofParentFolder.Id;
                    }
                    else
                    {
                        ...
                        templateModel.TemplateCode = "GENERAL_FOLDER";
                        var newmodel = await _noteBusiness.GetNoteDetails(templateModel);
                        newmodel.NoteSubject = lovDiscipline.Name;
                        newmodel.NoteStatusCode = "NOTE_STATUS_INPROGRESS";
                        newmodel.ParentNoteId = noteParentFolderId;
                        // newmodel.SequenceOrder = ;
                        newmodel.OwnerUserId = uc.UserId;
                        dynamic exo = new System.Dynamic.ExpandoObject();
                        ((IDictionary<String, Object>)exo).Add("Code", lovDiscipline.Name);
                        newmodel.Json = Newtonsoft.Json.JsonConvert.SerializeObject(exo);
                        var result = await _noteBusiness.ManageNote(newmodel);
                        if (result.IsSuccess)
                        {
                            noteDiscipline = result.Item.NoteId;
                        }
                    }
                }

                if (noteDiscipline.IsNotNullAndNotEmpty())
                {
                    var childfolderofDiscipline = await _noteBusiness.GetSingle(x => x.ParentNoteId == noteDiscipline && x.NoteSubject == lovDocCat.Name);
                    if (childfolderofDiscipline.IsNotNull())
                    {
                        noteDocCatId = childfolderofDiscipline.Id;
                    }
                    else
                    {
                        ...
                        newmodel.NoteSubject = lovDocCat.Name;
                        newmodel.NoteStatusCode = "NOTE_STATUS_INPROGRESS";
                        newmodel.ParentNoteId = noteDiscipline;
                        // newmodel.SequenceOrder = ;
                        newmodel.OwnerUserId = uc.UserId;
                        dynamic exo = new System.Dynamic.ExpandoObject();
                        ((IDictionary<String, Object>)exo).Add("Code", lovDocCat.Name);
        ...
        public async Task<CommandResult<TaskTemplateViewModel>> OverwriteEPEComment(TaskTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
          
            var _noteBusiness = sp.GetService<INoteBusiness>();
            string documentId = udf.documentId;
            string epeComment = udf.EPEComment;                     
            if (documentId.IsNotNullAndNotEmpty())
            {
                var noteDetails = await _noteBusiness.GetSingleById(documentId);
                var noteTempModel = new NoteTemplateViewModel();
                ...
                var notemodel = await _noteBusiness.GetNoteDetails(noteTempModel);
                var rowData1 = notemodel.ColumnList.ToDictionary(x => x.Name, x => x.Value);
                rowData1["EPEComment"] = epeComment;               
                var data1 = Newtonsoft.Json.JsonConvert.SerializeObject(rowData1);
                var update = await _noteBusiness.EditNoteUdfTable(notemodel, data1, notemodel.UdfNoteTableId);
            }
            return CommandResult<TaskTemplateViewModel>.Instance(viewModel);
