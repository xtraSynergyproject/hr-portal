using Synergy.App.Business.Interface.BusinessScript.Service.General.CoreHR;
...
namespace Synergy.App.Business.Implementations.BusinessScript.Service.General.CoreHR
{
    public class ServiceGeneralCoreHRPostScript : IServiceGeneralCoreHRPostScript
    {
        ...
        public async Task<CommandResult<ServiceTemplateViewModel>> TriggerNewPersonRequestService(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            ...
            else
            {
                ...
                if (userResult.IsSuccess)
                {
                    ...
                    perModel.TitleId = udf.TitleId;
                    perModel.FirstName = udf.FirstName;
                    perModel.LastName = udf.LastName;
                    perModel.ContactPersonalEmail = email;
                    perModel.GenderId = udf.GenderId;
                    perModel.DateOfBirth = Convert.ToDateTime(udf.DateOfBirth);
                    perModel.MaritalStatusId = udf.MaritalStatusId;
                    perModel.NationalityId = udf.NationalityId;
                    //perModel.ReligionId = udf.ReligionId;
                    perModel.DateOfJoin = Convert.ToDateTime(udf.DateOfJoin);
                    perModel.BusinessHierarchyParentId = udf.BusinessHierarchyParentId;
                    perModel.BusinessHierarchyReferenceType = udf.BusinessHierarchyReferenceType;
                    ...
                    if (perResult.IsSuccess)
                    {
                        ...
                        assignModel.DepartmentId = udf.DepartmentId;
                        assignModel.JobId = udf.JobId;
                        assignModel.PositionId = udf.PositionId;
                        assignModel.LocationId = udf.LocationId;
                        assignModel.AssignmentGradeId = udf.AssignmentGradeId;
                        assignModel.AssignmentTypeId = udf.AssignmentTypeId;
                        assignModel.AssignmentStatusId = assignstatusid;
                        assignModel.CareerLevelId = udf.CareerLevelId;
                        assignModel.DateOfJoin = udf.DateOfJoin;
        ...
        /// <summary>
        /// PostTravelReimbursementToPayroll
        /// </summary>
        /// <param name="viewModel"></param>
        /// <param name="udf"></param>
        /// <param name="uc"></param>
        /// <param name="sp"></param>
        /// <returns></returns>
        public async Task<CommandResult<ServiceTemplateViewModel>> PostTravelReimbursementToPayroll(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _payrollTransactionBusiness = sp.GetService<IPayrollTransactionsBusiness>();

            double reimbAmt = udf.ReimbursementAmount != null ? Convert.ToDouble(udf.ReimbursementAmount) : 0.00;
            ...
            if (viewModel.ServiceStatusCode == "SERVICE_STATUS_COMPLETE")
            {
                if (viewModel.OwnerUserId != null)
                {
                    ...
                    //    effectiveDate = effectiveDate.AddMonths(1).FirstDateOfMonth();
                    //}
                    //ReimbursementAmount Posting to Payroll
                    if (reimbAmt > 0)
                    {
                        var paymodel = await GeneratePayrollTransactionViewModel(reimbAmt, effectiveDate, "REIMB_TRAVEL", viewModel, uc, sp);
                        await _payrollTransactionBusiness.ManagePayrollTransaction(paymodel);
                    }
        ...
        /// <summary>
        /// PostMedicalReimbursementToPayroll
        /// </summary>
        /// <param name="viewModel"></param>
        /// <param name="udf"></param>
        /// <param name="uc"></param>
        /// <param name="sp"></param>
        /// <returns></returns>
        public async Task<CommandResult<ServiceTemplateViewModel>> PostMedicalReimbursementToPayroll(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _payrollTransactionBusiness = sp.GetService<IPayrollTransactionsBusiness>();
            ...
            if (viewModel.ServiceStatusCode == "SERVICE_STATUS_COMPLETE")
            {
                if (viewModel.OwnerUserId != null)
                {
                    ...
                    if (reimbAmt > 0)
                    {
                        var paymodel = await GeneratePayrollTransactionViewModel(reimbAmt, effectiveDate, "REIMB_MDECIAL", viewModel, uc, sp);
                        await _payrollTransactionBusiness.ManagePayrollTransaction(paymodel);
                    }
        ...
        /// <summary>
        /// PostEducationalReimbursementToPayroll
        /// </summary>
        /// <param name="viewModel"></param>
        /// <param name="udf"></param>
        /// <param name="uc"></param>
        /// <param name="sp"></param>
        /// <returns></returns>
        public async Task<CommandResult<ServiceTemplateViewModel>> PostEducationalReimbursementToPayroll(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _payrollTransactionBusiness = sp.GetService<IPayrollTransactionsBusiness>();
            ...
            if (viewModel.ServiceStatusCode == "SERVICE_STATUS_COMPLETE")
            {
                if (viewModel.OwnerUserId != null)
                {
                    ...
                    if (reimbAmt > 0)
                    {
                        var paymodel = await GeneratePayrollTransactionViewModel(reimbAmt, effectiveDate, "REIMB_EDUCATIONAL", viewModel, uc, sp);
                        await _payrollTransactionBusiness.ManagePayrollTransaction(paymodel);
                    }
        ...
        /// <summary>
        /// PostOtherReimbursementToPayroll
        /// </summary>
        /// <param name="viewModel"></param>
        /// <param name="udf"></param>
        /// <param name="uc"></param>
        /// <param name="sp"></param>
        /// <returns></returns>
        public async Task<CommandResult<ServiceTemplateViewModel>> PostOtherReimbursementToPayroll(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            var _payrollTransactionBusiness = sp.GetService<IPayrollTransactionsBusiness>();
            ...
            if (viewModel.ServiceStatusCode == "SERVICE_STATUS_COMPLETE")
            {
                if (viewModel.OwnerUserId != null)
                {
                    ...
                    if (reimbAmt > 0)
                    {
                        var paymodel = await GeneratePayrollTransactionViewModel(reimbAmt, effectiveDate, "REIMB_OTHERS", viewModel, uc, sp);
                        await _payrollTransactionBusiness.ManagePayrollTransaction(paymodel);
                    }
        ...
        public async Task<CommandResult<ServiceTemplateViewModel>> PostCRPFTransferRequest(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            return CommandResult<ServiceTemplateViewModel>.Instance(viewModel);
        }
        ...
        public async Task<CommandResult<ServiceTemplateViewModel>> AddStockIndividualTransferDetails(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            udf.EmployeeId = uc.UserId;
            var _noteBusiness = sp.GetService<INoteBusiness>();
            var _serviceBusinness = sp.GetService<IServiceBusiness>();
            var _inventoryManagementBusinness = sp.GetService<IInventoryManagementBusiness>();
            var _hrCoreBusiness = sp.GetService<IHRCoreBusiness>();
            //  var data = await _inventoryManagementBusinness.GetItemTransferredList("", "", udf.ChallanNo);
            var data = await _hrCoreBusiness.GeEmployeeFormTransferItemList(udf.EmployeeId);
            if (data != null)
            {
                if (viewModel.DataAction == DataActionEnum.Create)
                {
                    ...
                    sermodel.Json = Newtonsoft.Json.JsonConvert.SerializeObject(viewModel);
        ...
        public async Task<CommandResult<ServiceTemplateViewModel>> AddStockTransferDetails(ServiceTemplateViewModel viewModel, dynamic udf, IUserContext uc, IServiceProvider sp)
        {
            ...
            var _hrCoreBusiness = sp.GetService<IHRCoreBusiness>();
            //  var data = await _inventoryManagementBusinness.GetItemTransferredList("", "", udf.ChallanNo);
            var data = await _hrCoreBusiness.GeEmployeeFormTransferItemList(udf.EmployeeId).toList();
            if(data != null)
            {
                if (viewModel.DataAction == DataActionEnum.Create)
                {
