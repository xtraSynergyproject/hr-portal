...
namespace Synergy.App.Business
{
    public class HRCoreBusiness : BusinessBase<NoteViewModel, NtsNote>, IHRCoreBusiness
    {
        ...
        private readonly IHRCoreQueryBusiness _hRCoreQueryBusiness;
        ...
        ILegalEntityBusiness legalEntityBusiness, IUserInfoBusiness userInfoBusiness,
            ...
            IRepositoryQueryBase<BusinessHierarchyAORViewModel> queryRepoAOR,
             ...
             IRepositoryQueryBase<UserHierarchyPermissionViewModel> userHierarchyPermission,
             IHRCoreQueryBusiness hRCoreQueryBusiness) : base(repo, autoMapper)
...

        {
            ...
            _hRCoreQueryBusiness = hRCoreQueryBusiness;
        }

        public async Task<OrganizationChartIndexViewModel> GetOrgHierarchyParentId(string personId)
        {
            var list = await _hRCoreQueryBusiness.GetOrgHierarchyParentId(personId); //await _queryRepo2.ExecuteQuerySingle(query, null);
            return list;
        }
        public async Task<List<OrganizationChartViewModel>> GetOrgHierarchy(string parentId, int levelUpto)
        {
            var list = await _hRCoreQueryBusiness.GetOrgHierarchy(parentId, levelUpto); //await _queryRepo2.ExecuteQuerySingle(query, null);
            return list;
        }
        public async Task<PositionChartIndexViewModel> GetPostionHierarchyParentId(string personId)
        {

            var list = await _hRCoreQueryBusiness.GetPostionHierarchyParentId(personId); //await _queryRepo2.ExecuteQuerySingle(query, null);
            return list;

        }

        public async Task<List<PositionChartViewModel>> GetPositionHierarchy(string parentId, int levelUpto, bool? enableTask = null, string categoryCodes = null, string templateCodes = null, string portalNames = null)
        {
            string query = $@"  select distinct d.""Id"" as Id,d.""NtsNoteId"" as NoteId,d.""PositionName"" as PositionName ,c.""Id"" as JobId,p.""NtsNoteId"" as PersonNoteId,a.""NtsNoteId"" as AssignmentNoteId,c.""NtsNoteId"" as JobNoteId, c.""JobTitle"" as JobName, x.""Id"" as OrganizationId, x.""DepartmentName"" as OrganizationName
                            ,p.""Id"" as PersonId,h.""ParentPositionId"" as ParentId,coalesce(t.""Count"",0) as DirectChildCount,u.""PhotoId"" as PhotoId,coalesce(p.""PersonFullName"",u.""Name"") as DisplayName,case when u.""Id"" is not null then true else false end as HasUser
            ...
            var taskWhere = "";
            var portal = "";
            if (enableTask.IsTrue())
            {
                taskCount = $@",tsk.""TasksPendingCount"" as ""TasksPendingCount"",tsk.""TasksCompletedCount"" as ""TasksCompletedCount""";
                taskJoin = $@"left join (
                select 
                sum(case when l.""Code"" = 'TASK_STATUS_INPROGRESS' or l.""Code"" = 'TASK_STATUS_OVERDUE' then 1 else 0 end) as ""TasksPendingCount"",
                sum(case when l.""Code"" = 'TASK_STATUS_COMPLETE' or l.""Code"" = 'TASK_STATUS_REJECT' or l.""Code"" = 'TASK_STATUS_CANCEL' then 1 else 0 end) as ""TasksCompletedCount"",
                ...
                join public.""TemplateCategory"" as stc on tem.""TemplateCategoryId""=stc.""Id"" and stc.""IsDeleted""=false  
                <<portal>>
                join public.""LOV"" as l on l.""Id""=t.""TaskStatusId"" and l.""IsDeleted""=false  
                where t.""IsDeleted""=false <<task-where>> group by t.""AssignedToUserId""
                )tsk on tsk.""AssignedToUserId""=u.""Id""";

                if (portalNames.IsNotNullAndNotEmpty())
                {
                    portalNames = $"'{portalNames.Replace(",", "','")}'";
                    portal = @$" join public.""Portal"" port on port.""Name"" in ({portalNames}) and port.""IsDeleted""=false and  port.""Id""=ANY(stc.""AllowedPortalIds"")";
                }
                if (templateCodes.IsNotNullAndNotEmpty())
                {
                    templateCodes = templateCodes.Replace(",", "','");
                    templateCodes = String.Concat("'", templateCodes, "'");
                    taskWhere = @" and tem.""Code"" in (" + templateCodes + ") ";
                }
                if (categoryCodes.IsNotNullAndNotEmpty())
                {
                    categoryCodes = categoryCodes.Replace(",", "','");
                    categoryCodes = String.Concat("'", categoryCodes, "'");
                    taskWhere = @" and stc.""Code"" in (" + categoryCodes + ") ";
                }

            }

           query =  query.Replace("<<task-count>>", taskCount).Replace("<<task-join>>", taskJoin)
                .Replace("<<portal>>", portal).Replace("<<task-where>>", taskWhere);
        ...
        public async Task<PagedList<IdNameViewModel>> GetPersonFilterdList(string filter, int pageSize, int pageNumber, string indexedItemId = null)
        {
            //string query = $@"select p.""Id"" as Id ,CONCAT( p.""FirstName"",' ',p.""LastName"") as Name
            //                from cms.""N_CoreHR_HRPerson"" as p    
            //                where p.""IsDeleted""=false and p.""CompanyId""='{_userContext.CompanyId}'
            ...
            string baseQuery = $@" from cms.""N_CoreHR_HRPerson"" as p    
...
";
            ...
            {
                where = $@" and CONCAT( p.""FirstName"",' ',p.""LastName"") ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu""  ";
            }
            baseQuery = baseQuery.Replace("#WHERE#", where);

            string query1 = $@" Select Count(p.""Id"") {baseQuery}";
            string query2 = $@" select p.""Id"" as Id ,CONCAT( p.""FirstName"",' ',p.""LastName"") as Name
 {baseQuery} order by p.""FirstName""   OFFSET {offset}  LIMIT {pageSize}";
            if (indexedItemId.IsNotNullAndNotEmpty())
            {
                var indexQuery = $@"With cte as (select p.""Id"", ROW_NUMBER () OVER (ORDER BY p.""FirstName"",p.""Id"") as rowNumber
{baseQuery} order by p.""FirstName"" )select ""rowNumber"" from cte where ""Id""='{indexedItemId}' ";
                var index = await _queryRepo1.ExecuteScalar<long>(indexQuery, null);
                return PagedList<IdNameViewModel>.Instance(index);
                // ROW_NUMBER() OVER(ORDER BY product_id)
            }
            var total = await _queryRepo1.ExecuteScalar<long>(query1, null);
            var list2 = await _queryRepo1.ExecuteQueryList(query2, null);
            return PagedList<IdNameViewModel>.Instance(total, list2);
        }
...
substring(assi.""DateOfJoin"",0,11)::TIMESTAMP::DATE as DateOfJoin,assi.""DateOfJoin"" as DTDateOfJoin,lovs.""Name"" as AssignmentStatusName,at.""Name"" as AssignmentTypeName,case when hp.""Status""=1 then 'Active' else 'InActive' end as PersonStatus,
case when u.""Status""=1 then 'Active' else 'InActive' end as UserStatus,u.""Id"" as UserId,nt.""Id"" as NoteId,assi.""Id"" as AssignmentId,assi.""NoticePeriod"",
cont.""Id"" as ContractId,ntc.""Id"" as NoteContractId,nta.""Id"" as NoteAssignmentId,hpos.""Id"" as PositionId,pp.""Name"" as ProbationPeriodName,
ph.""Id"" as PositionHierarchyId,hm.""Id"" as HierarchyId,
ntp.""Id"" as NotePositionHierarchyId,
si.""Id"" as SalaryInfoId,ntsi.""Id"" as NoteSalaryInfoId,u.""PhotoId"" as PhotoId,u.""PhotoId"" as PhotoName,
hd.""Id"" as DepartmentId,hj.""Id"" as JobId,hp.""Id"" as PersonId,u.""Email"" as Email,hp.""PersonNo"" as PersonNo,u.""Mobile"" as Mobile,
hp.*,u1.""Name"" as ""ManagerPersonFullName"",u1.""PhotoId"" as ""ManagerPhoto"",u1.""JobTitle"" as ""ManagerJobName""
...
left join public.""User"" as u1 on u1.""LineManagerId""=u.""Id"" and u1.""IsDeleted""=false
...
left join public.""LOV"" as at on assi.""AssignmentTypeId""=at.""Id"" and at.""IsDeleted""=false
left join public.""LOV"" as pp on assi.""ProbationPeriodId""=pp.""Id"" and pp.""IsDeleted""=false
...
where hp.""IsDeleted""=false #WHERE#
        ...
        public async Task<PagedList<AssignmentViewModel>> GetPersonAssignmentDetails(string filter, int pageSize, int pageNumber, string indexedItemId = null, string legalEntityId = null)
        {
            #region
            //            string query = $@"select CONCAT( hp.""FirstName"",' ',hp.""LastName"") as PersonFullName,hd.""DepartmentName"",hj.""JobTitle"" as JobName,hpos.""PositionName"",hl.""LocationName"",hg.""GradeName"",
            ...
            int offset = (pageNumber - 1) * pageSize;
            string baseQuery = $@" from cms.""N_CoreHR_HRPerson"" as hp
left join public.""NtsNote"" as nt on nt.""Id""=hp.""NtsNoteId"" and nt.""IsDeleted""=false
left join cms.""N_CoreHR_HRAssignment"" as assi on hp.""Id""=assi.""EmployeeId"" and assi.""IsDeleted""=false
left join public.""NtsNote"" as nta on nta.""Id""=assi.""NtsNoteId"" and nta.""IsDeleted""=false
left join public.""User"" as u on u.""Id""=hp.""UserId"" and u.""IsDeleted""=false
...
";
            ...
            if (filter.IsNotNullAndNotEmpty())
            {
                where = $@" and (CONCAT( hp.""FirstName"",' ',hp.""LastName"") ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu"" or hd.""DepartmentName"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu"" or hj.""JobTitle"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu""
or hpos.""PositionName"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu"" or hg.""GradeName"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu""  or u.""Email"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu"" 
or hp.""PersonNo"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu"" or hp.""PersonNo"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu"" or hp.""SponsorshipNo"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu"" )  ";
            }
            baseQuery = baseQuery.Replace("#WHERE#", where);

            string query1 = $@" Select Count(hp.""Id"") {baseQuery}";
            string query2 = $@" select CONCAT( hp.""FirstName"",' ',hp.""LastName"") as PersonFullName,hd.""DepartmentName"",hj.""JobTitle"" as JobName,hpos.""PositionName"",hl.""LocationName"",hg.""GradeName"",
substring(assi.""DateOfJoin"",0,11) as DateOfJoin,lovs.""Name"" as AssignmentStatusName,case when hp.""Status""=1 then 'Active' else 'InActive' end as PersonStatus,
case when u.""Status""=1 then 'Active' else 'InActive' end as UserStatus,u.""Id"" as UserId,nt.""Id"" as NoteId,assi.""Id"" as AssignmentId,
cont.""Id"" as ContractId,ntc.""Id"" as NoteContractId,nta.""Id"" as NoteAssignmentId,hpos.""Id"" as PositionId,
ph.""Id"" as PositionHierarchyId,hm.""Id"" as HierarchyId,
ntp.""Id"" as NotePositionHierarchyId,
si.""Id"" as SalaryInfoId,ntsi.""Id"" as NoteSalaryInfoId,u.""PhotoId"" as PhotoName,
hd.""Id"" as DepartmentId,hj.""Id"" as JobId,hp.""Id"" as PersonId,u.""Email"" as Email,hp.""PersonNo"" as PersonNo,
hp.*
 {baseQuery} order by CONCAT( hp.""FirstName"",' ',hp.""LastName"")   OFFSET {offset}  LIMIT {pageSize}";
            if (indexedItemId.IsNotNullAndNotEmpty())
            {
                var indexQuery = $@"With cte as (select hp.""Id"", ROW_NUMBER () OVER (ORDER BY CONCAT( hp.""FirstName"",' ',hp.""LastName""),hp.""Id"") as rowNumber
{baseQuery} order by CONCAT( hp.""FirstName"",' ',hp.""LastName"") )select rowNumber from cte where ""Id""='{indexedItemId}' ";
                var index = await _queryAssignment.ExecuteScalar<long>(indexQuery, null);
                return PagedList<AssignmentViewModel>.Instance(index);
                // ROW_NUMBER() OVER(ORDER BY product_id)
            }
            var total = await _queryAssignment.ExecuteScalar<long>(query1, null);
            var list2 = await _queryAssignment.ExecuteQueryList(query2, null);
            return PagedList<AssignmentViewModel>.Instance(total, list2);
        }
...
join cms.""N_CoreHR_HRPerson"" as p on a.""EmployeeId""=p.""Id"" and p.""IsDeleted""=false and p.""CompanyId""='{_userContext.CompanyId}'
        ...
        public async Task<PagedList<IdNameViewModel>> GetFilteredPersonListByOrgId(string filter, int pageSize, int pageNumber, string indexedItemId = null)
        {
            //            string query = @$"select p.""Id"" as Id,p.""PersonFullName"" as Name
            //from cms.""N_CoreHR_HRDepartment"" as d
            //join cms.""N_CoreHR_HRAssignment"" as a on a.""DepartmentId""=d.""Id"" and a.""IsDeleted""=false and a.""CompanyId""='{_userContext.CompanyId}'
            ...
            //            return result;
            int offset = (pageNumber - 1) * pageSize;
            string baseQuery = $@" from cms.""N_CoreHR_HRDepartment"" as d
            join cms.""N_CoreHR_HRAssignment"" as a on a.""DepartmentId""=d.""Id"" and a.""IsDeleted""=false and a.""CompanyId""='{_userContext.CompanyId}'
            join cms.""N_CoreHR_HRPerson"" as p on a.""EmployeeId""=p.""Id"" and p.""IsDeleted""=false and p.""CompanyId""='{_userContext.CompanyId}'
                                            where  d.""IsDeleted""=false and d.""CompanyId""='{_userContext.CompanyId}'  #WHERE# 
";
            var where = "";

            if (filter.IsNotNullAndNotEmpty())
            {
                where = $@" and p.""PersonFullName"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu""   ";
            }
            baseQuery = baseQuery.Replace("#WHERE#", where);

            string query1 = $@" Select Count(p.""Id"") {baseQuery}";
            string query2 = $@"select p.""Id"" as Id,p.""PersonFullName"" as Name
 {baseQuery} order by p.""PersonFullName""   OFFSET {offset}  LIMIT {pageSize}";
            if (indexedItemId.IsNotNullAndNotEmpty())
            {
                var indexQuery = $@"With cte as (select p.""Id"", ROW_NUMBER () OVER (ORDER BY p.""PersonFullName"",p.""Id"") as ""rowNumber""
{baseQuery} order by p.""PersonFullName"" ) select ""rowNumber"" from cte where ""Id""='{indexedItemId}' ";
                var index = await _queryRepo1.ExecuteScalar<long>(indexQuery, null);
                return PagedList<IdNameViewModel>.Instance(index);
                // ROW_NUMBER() OVER(ORDER BY product_id)
            }
...
left join public.""User"" as u on p.""UserId""=u.""Id"" and u.""IsDeleted""=false
        ...
        {
            string query = @$"SELECT c.""AnnualLeaveEntitlement"" as AnnualLeaveEntitlement,ct.""Name"" as ContractType,cr.""Name"" as ""ContractRenewable"",s.""SponsorName"",c.""NtsNoteId"" as NoteId
                                FROM cms.""N_CoreHR_HRContract"" as c
                                join public.""LOV"" as ct on c.""ContractTypeId""=ct.""Id""
                                join public.""LOV"" as cr on c.""ContractRenewable""=cr.""Id""
                                join cms.""N_CoreHR_HRSponsor"" as s on c.""SponsorId""=s.""Id"" and s.""IsDeleted""=false and s.""CompanyId""='{_userContext.CompanyId}'
                              where c.""EmployeeId""='{personId}' and c.""IsDeleted""=false and c.""CompanyId""='{_userContext.CompanyId}'
                            ";
            var result = await _queryAssignment.ExecuteQuerySingle<AssignmentViewModel>(query, null);
            return result;
        }
...
,""NtsService"".""ServiceNo"",""NtsService"".""Id"" as ""ServiceId"",""NtsService"".""CreatedDate"" as ""CreatedDate"",""NtsService"".""ServiceSubject"",""NtsService"".""ServiceDescription"", ""NtsService"".""StartDate"" as AppliedDate FROM cms.""{tableMeta.Name}"" as ""T""
 ...
 left join  (select ""NtsService"".""Id"", ""NtsService"".""ParentServiceId"",""NtsService"".""ServiceNo"" from cms.""N_Leave_Leave-Handover-Service"" as  ""T"" 
	...
	where ""OwnerUser"".""Id""='{userId}') as HO on HO.""ParentServiceId""=""NtsService"".""Id""
        ...
        public async Task<List<TravelReimbursementViewModel>> GetTravelReimbursementbyOwneruserId(string Id)
        {
            var query = "";


            query = $@"SELECT NS.""Id"" as NtsNoteId, B.""Id"",  LOV.""Name"" as Status,LOV.""Code"" as StatusCode, U.""Name"" as UserName ,
            B.""ReimbursementDetails"",NS.""StartDate""::TIMESTAMP::DATE,NS.""ServiceNo"",NS.""DueDate""::TIMESTAMP::DATE,B.""EventDate""::TIMESTAMP::DATE as EventDate
            FROM  public.""NtsService"" NS
            inner join public.""NtsNote"" N on Ns.""UdfNoteId""=N.""Id"" and N.""IsDeleted""=false and N.""CompanyId""='{_userContext.CompanyId}'
	        inner join  cms.""N_SNC_CHR_REIMBURSEMENT_TravelReimbursement"" B on  N.""Id"" =B.""NtsNoteId"" and	 B.""IsDeleted""=false and B.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""LOV"" as LOV on NS.""ServiceStatusId""=LOV.""Id"" and  LOV.""IsDeleted""=false and LOV.""CompanyId""='{_userContext.CompanyId}'
            left join public.""NtsService"" as cs on cs.""ParentServiceId""=NS.""Id"" and cs.""IsDeleted""=false and cs.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""User"" as U on U.""Id""='{Id}' and U.""IsDeleted""=false and U.""CompanyId""='{_userContext.CompanyId}'
            where N.""OwnerUserId""='{Id}' and  NS.""IsDeleted""=false and NS.""CompanyId""='{_userContext.CompanyId}' order by NS.""CreatedDate"" desc";
            var queryData = await _queryBusinessTrip.ExecuteQueryList<TravelReimbursementViewModel>(query, null);

            var list = new List<TravelReimbursementViewModel>();
            //  var list1 = queryData.GroupBy(x => x.NtsStatus).Select(group => new { Value = group.Count(), Type = group.Key, Id = group.Select(x => x.NtsStatusId).FirstOrDefault() }).ToList();
            list = queryData.Select(x => new TravelReimbursementViewModel { NtsNoteId = x.NtsNoteId, Id = x.Id, Status = x.Status, ServiceNo = x.ServiceNo, EventDate = x.EventDate, ReimbursementDetails = x.ReimbursementDetails, DueDate = x.DueDate, StartDate = x.StartDate, ReimbursementAmount = x.ReimbursementAmount, UserName = x.UserName }).ToList();
            return list;


        }

        public async Task<List<TravelReimbursementViewModel>> GetMedicalReimbursementbyOwneruserId(string Id)
        {
            var query = "";
            ...
            FROM  public.""NtsService"" NS  
            inner join public.""NtsNote"" N on Ns.""UdfNoteId""=N.""Id"" and N.""IsDeleted""=false and N.""CompanyId""='{_userContext.CompanyId}'
	        inner join  cms.""N_SNC_CHR_REIMBURSEMENT_MedicalReimbursement"" B on  N.""Id"" =B.""NtsNoteId"" and	 B.""IsDeleted""=false and B.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""LOV"" as LOV on NS.""ServiceStatusId""=LOV.""Id"" and  LOV.""IsDeleted""=false and LOV.""CompanyId""='{_userContext.CompanyId}'
            left join public.""NtsService"" as cs on cs.""ParentServiceId""=NS.""Id"" and cs.""IsDeleted""=false and cs.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""User"" as U on U.""Id""='{Id}' and U.""IsDeleted""=false and U.""CompanyId""='{_userContext.CompanyId}'
            where N.""OwnerUserId""='{Id}' and  NS.""IsDeleted""=false and NS.""CompanyId""='{_userContext.CompanyId}' order by NS.""CreatedDate"" desc";
        ...
        public async Task<List<HRPolicyViewModel>> GetPolicyDocs(string Id)
        {
            var query = "";


            query = $@"SELECT N.""Id"" as NtsNoteId, B.""Id"", B.""PolicyName"", B.""PolicyDescription"", B.""PolicyDocument""   
            , N.""StartDate""::TIMESTAMP::DATE
            FROM  public.""NtsNote"" as N  
	        inner join  cms.""N_CoreHR_HRPolicy"" as B on  N.""Id"" = B.""NtsNoteId"" and B.""IsDeleted""=false and B.""CompanyId""='{_userContext.CompanyId}'
            where --N.""OwnerUserId""='{Id}' and  
N.""IsDeleted""=false and N.""CompanyId""='{_userContext.CompanyId}' order by N.""CreatedDate"" desc";
            var queryData = await _queryBusinessTrip.ExecuteQueryList<HRPolicyViewModel>(query, null);
            return queryData;


        }

        public async Task<List<TravelReimbursementViewModel>> GetEducationalReimbursementbyOwneruserId(string Id)
        {
            var query = "";
            ...
            FROM  public.""NtsService"" NS  
            inner join public.""NtsNote"" N on Ns.""UdfNoteId""=N.""Id"" and N.""IsDeleted""=false and N.""CompanyId""='{_userContext.CompanyId}'
	        inner join  cms.""N_SNC_CHR_REIMBURSEMENT_EducationalReimbursement"" B on  N.""Id"" =B.""NtsNoteId"" and	 B.""IsDeleted""=false and B.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""LOV"" as LOV on NS.""ServiceStatusId""=LOV.""Id"" and  LOV.""IsDeleted""=false and LOV.""CompanyId""='{_userContext.CompanyId}'
            left join public.""NtsService"" as cs on cs.""ParentServiceId""=NS.""Id"" and cs.""IsDeleted""=false and cs.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""User"" as U on U.""Id""='{Id}' and U.""IsDeleted""=false and U.""CompanyId""='{_userContext.CompanyId}'
            where N.""OwnerUserId""='{Id}' and  NS.""IsDeleted""=false and NS.""CompanyId""='{_userContext.CompanyId}' order by NS.""CreatedDate"" desc";
        ...
        public async Task<List<TravelReimbursementViewModel>> GetOtherReimbursementbyOwneruserId(string Id)
        {
            var query = "";
            ...
            FROM  public.""NtsService"" NS  
            inner join public.""NtsNote"" N on Ns.""UdfNoteId""=N.""Id"" and N.""IsDeleted""=false and N.""CompanyId""='{_userContext.CompanyId}'
	        inner join  cms.""N_SNC_CHR_REIMBURSEMENT_OtherReimbursement"" B on  N.""Id"" =B.""NtsNoteId"" and	 B.""IsDeleted""=false and B.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""LOV"" as LOV on NS.""ServiceStatusId""=LOV.""Id"" and  LOV.""IsDeleted""=false and LOV.""CompanyId""='{_userContext.CompanyId}'
            left join public.""NtsService"" as cs on cs.""ParentServiceId""=NS.""Id"" and cs.""IsDeleted""=false and cs.""CompanyId""='{_userContext.CompanyId}'
            inner join public.""User"" as U on U.""Id""='{Id}' and U.""IsDeleted""=false and U.""CompanyId""='{_userContext.CompanyId}'
            where N.""OwnerUserId""='{Id}' and  NS.""IsDeleted""=false and NS.""CompanyId""='{_userContext.CompanyId}' order by NS.""CreatedDate"" desc";
        ...
        public async Task<List<ServiceViewModel>> GetTransferDetails(string Id)
        {
            var query = "";
            query = $@"SELECT s.""ServiceNo"", s.""CreatedDate""::TIMESTAMP::DATE, s.""Id"", ss.""Name"" as ServiceStatusName
                            ,l.""LocationName"" as Location
                            FROM public.""NtsService"" s
                            join cms.""N_SNC_CHR_TransferRequest"" as tr on s.""UdfNoteTableId""=tr.""Id"" and tr.""IsDeleted""=false
                            join cms.""N_CoreHR_HRLocation"" as l on tr.""LocationId""=l.""Id"" and l.""IsDeleted""=false
                            inner join public.""LOV"" as ss on s.""ServiceStatusId""=ss.""Id"" and ss.""IsDeleted""=false and ss.""CompanyId""='{_userContext.CompanyId}'
                            where s.""TemplateCode""='PIS_TRANSFER_REQUEST' and s.""OwnerUserId""='{Id}' and s.""IsDeleted""=false and s.""CompanyId""='{_userContext.CompanyId}' order by s.""CreatedDate"" desc ";
...
where ""IsDeleted""=false and ""CompanyId""='{_userContext.CompanyId}')) or dep.""Id""='{deptId}' ";
        ...
        public async Task<PagedList<IdNameViewModel>> GetNonExistingDepartmentList(string hierarchyId, string excludingPositionId, string filter, int pageSize, int pageNumber, string indexedItemId = null)
        {
            int offset = (pageNumber - 1) * pageSize;
            var companyquery = Helper.OrganizationMapping(_userContext.UserId, _userContext.CompanyId, _userContext.LegalEntityId);
            string baseQuery = $@" from cms.""N_CoreHR_HRDepartment"" as dep
                            join  ""Department"" as dept on dept.""DepartmentId""=dep.""Id""                          
where dep.""IsDeleted""=false and  dep.""CompanyId""='{_userContext.CompanyId}' and dep.""ParentDepartmentId"" is null and dep.""Id"" not in(SELECT ""RootNodeId"" FROM public.""HierarchyMaster"" 
where ""Id""='{hierarchyId}' and ""IsDeleted""=false and ""CompanyId""='{_userContext.CompanyId}')   #WHERE# #EXCLUDE#
";
            var where = "";
            var exclude = "";
            if (filter.IsNotNullAndNotEmpty())
            {
                where = $@" and dep.""DepartmentName"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu""";
            }
            baseQuery = baseQuery.Replace("#WHERE#", where);
            if (excludingPositionId.IsNotNullAndNotEmpty())
            {
                exclude = $@" and dep.""Id"" <> '{excludingPositionId}'";
            }
            baseQuery = baseQuery.Replace("#EXCLUDE#", exclude);
            string query1 = $@"{companyquery} Select Count(dep.""Id"") {baseQuery}";
            string query2 = $@"{companyquery} select dep.""DepartmentName"" as Name,dep.""Id"" as Id
 {baseQuery} order by dep.""DepartmentName""   OFFSET {offset}  LIMIT {pageSize}";
            if (indexedItemId.IsNotNullAndNotEmpty())
            {
                var indexQuery = $@"With cte as (select dep.""Id"", ROW_NUMBER () OVER (ORDER BY dep.""DepartmentName"",dep.""Id"") as rowNumber
{baseQuery} order by dep.""DepartmentName"" )select ""rowNumber"" from cte where ""Id""='{indexedItemId}' ";
                var index = await _queryRepo1.ExecuteScalar<long>(indexQuery, null);
                return PagedList<IdNameViewModel>.Instance(index);
                // ROW_NUMBER() OVER(ORDER BY product_id)
            }
...
";
        ...
        public async Task<PagedList<IdNameViewModel>> GetNonExistingPositionList(string hierarchyId, string excludingPositionId, string filter, int pageSize, int pageNumber, string indexedItemId = null)
        {
            int offset = (pageNumber - 1) * pageSize;
            var companyquery = Helper.OrganizationMapping(_userContext.UserId, _userContext.CompanyId, _userContext.LegalEntityId);
            string baseQuery = $@" from cms.""N_CoreHR_HRPosition"" as dep
join  ""Department"" as dept on dept.""DepartmentId""=dep.""DepartmentId""
where dep.""IsDeleted""=false and  dep.""CompanyId""='{_userContext.CompanyId}' and dep.""ParentPositionId"" is null and dep.""Id"" not in(SELECT ""RootNodeId"" FROM public.""HierarchyMaster"" 
where ""Id""='{hierarchyId}' and ""IsDeleted""=false and ""CompanyId""='{_userContext.CompanyId}')  #WHERE# #EXCLUDE#
";
            ...
            if (filter.IsNotNullAndNotEmpty())
            {
                where = $@" and dep.""PositionName"" ILIKE '%{filter}%' COLLATE ""tr-TR-x-icu""";
            }
            baseQuery = baseQuery.Replace("#WHERE#", where);
            if (excludingPositionId.IsNotNullAndNotEmpty())
            {
            ...
            baseQuery = baseQuery.Replace("#EXCLUDE#", exclude);
            string query1 = $@"{companyquery} Select Count(dep.""Id"") {baseQuery}";
            string query2 = $@"{companyquery} select dep.""PositionName"" as Name,dep.""Id"" as Id
 {baseQuery} order by dep.""PositionName""   OFFSET {offset}  LIMIT {pageSize}";
            if (indexedItemId.IsNotNullAndNotEmpty())
            {
                var indexQuery = $@"With cte as (select dep.""Id"", ROW_NUMBER () OVER (ORDER BY dep.""PositionName"",dep.""Id"") as rowNumber
{baseQuery} order by dep.""PositionName"" )select ""rowNumber"" from cte where ""Id""='{indexedItemId}' ";
                var index = await _queryRepo1.ExecuteScalar<long>(indexQuery, null);
                return PagedList<IdNameViewModel>.Instance(index);
                // ROW_NUMBER() OVER(ORDER BY product_id)
            }
...
";
        ...
        public async Task<AssignmentViewModel> GetUserPerformanceDocumentInfo(string userId, string PerformanceId, string MasterStageId)
        {
            ...
            string query = $@"select   coalesce( hp.""PersonFullName"",u.""Name"") as PersonFullName,hd.""DepartmentName"",hj.""JobTitle"" as JobName
            ,hpos.""PositionName"",hl.""LocationName"",hg.""GradeName"",assi.""DateOfJoin"",assi.""Id"" as AssignmentId
            ,hpos.""Id"" as PositionId,u.""PhotoId"" as PhotoId,hd.""Id"" as DepartmentId,hj.""Id"" as JobId,hp.""Id"" as PersonId
            ,pd.""Id"" as PerformanceDocumentId,pd.""Name"" as PerformanceDocumentName,pd.""StartDate"" as PDStartDate,pd.""EndDate"" as PDEndDate
            ,pd.""Year"" as PDYear,ss.""Name"" as PDStatus,mu.""Id"" as ""ManagerUserId"",mu.""Name"" as ManagerPersonFullName,mu.""JobTitle"" as ManagerJobName
            ,c.""LogoFileId"" as SponsorLogoId,pds.""FinalRatingRounded"" as FinalScore,pds.""ManagerComment"" as FinalComments
            ,pds.""Name"" as PDStageName
            from cms.""N_PerformanceDocument_PMSPerformanceDocumentStage"" as pds
            join public.""Company"" as c on pds.""CompanyId""=c.""Id"" and c.""IsDeleted""=false  
            join public.""NtsService"" as pdss on pds.""NtsNoteId""=pdss.""UdfNoteId"" and pdss.""IsDeleted""=false and pdss.""CompanyId""='{_userContext.CompanyId}'
            join public.""LOV"" as ss on ss.""Id"" = pdss.""ServiceStatusId"" and ss.""IsDeleted""=false 
            join public.""User"" as u on pdss.""OwnerUserId""=u.""Id"" and u.""IsDeleted""=false and u.""CompanyId""='{_userContext.CompanyId}'
            join cms.""N_PerformanceDocumentMaster_PerformanceDocumentMasterStage"" as pdms on pdms.""Id""=pds.""DocumentMasterStageId"" and pdms.""IsDeleted""=false and pdms.""CompanyId""='{_userContext.CompanyId}'
            join cms.""N_PerformanceDocumentMaster_PerformanceDocumentMaster"" as pdm on pdm.""Id""=pds.""DocumentMasterId"" and pdm.""IsDeleted""=false and pdm.""CompanyId""='{_userContext.CompanyId}'
            join cms.""N_PerformanceDocument_PMSPerformanceDocument"" as pd on pd.""Id""=pds.""DocumentId"" and pd.""IsDeleted""=false and pd.""CompanyId""='{_userContext.CompanyId}'
            left join cms.""N_CoreHR_HRPerson"" as hp on hp.""UserId""=u.""Id"" and hp.""IsDeleted""=false and hp.""CompanyId""='{_userContext.CompanyId}'
            left join cms.""N_CoreHR_HRAssignment"" as assi on hp.""Id""=assi.""EmployeeId"" and assi.""IsDeleted""=false and assi.""CompanyId""='{_userContext.CompanyId}'
            left join cms.""N_CoreHR_HRDepartment"" as hd on hd.""Id""=assi.""DepartmentId"" and hd.""IsDeleted""=false and hd.""CompanyId""='{_userContext.CompanyId}'
            left join cms.""N_CoreHR_HRJob"" as hj on hj.""Id""=assi.""JobId"" and hj.""IsDeleted""=false and hj.""CompanyId""='{_userContext.CompanyId}'
            ...
            left join public.""LOV"" as sp on sp.""Id""=u.""SponsorId"" and sp.""IsDeleted""=false and sp.""CompanyId""='{_userContext.CompanyId}'
            left join (
                select h.""UserId"", h.""ParentUserId"" from public.""UserHierarchy"" h
                join public.""HierarchyMaster"" hm on h.""HierarchyMasterId""=hm.""Id"" and hm.""IsDeleted""=false and hm.""CompanyId""='{_userContext.CompanyId}'
                 where hm.""Code""='PERFORMANCE_HIERARCHY'
                 and h.""IsDeleted""=false and h.""CompanyId""='{_userContext.CompanyId}'
                 and h.""LevelNo""=1 and h.""OptionNo""=1
                )muh on muh.""UserId""=u.""Id""
            left join public.""User"" mu on muh.""ParentUserId""=mu.""Id""
            where u.""Id""='{userId}' and pds.""IsDeleted""=false and pds.""CompanyId""='{_userContext.CompanyId}' and pdms.""Id""='{MasterStageId}' ";
        ...
        public async Task<List<AssignmentViewModel>> GetUsersInfo(string deptId = null)
        {
            string query = $@"select u.""Name"" as PersonFullName,hd.""DepartmentName"",hj.""JobTitle"" as JobName,
                ...
                from 
                public.""User"" as u 
join public.""UserPortal"" as up on up.""UserId""=u.""Id"" and up.""IsDeleted""=false and up.""PortalId""='{_userContext.PortalId}'
                 left join cms.""N_CoreHR_HRPerson"" as hp on hp.""UserId""=u.""Id"" and hp.""IsDeleted""=false and hp.""CompanyId""='{_userContext.CompanyId}'
                 left join cms.""N_CoreHR_HRAssignment"" as assi on hp.""Id""=assi.""EmployeeId"" and assi.""IsDeleted""=false and assi.""CompanyId""='{_userContext.CompanyId}'
                 left join public.""LOV"" as lovs on lovs.""Id""=assi.""AssignmentStatusId"" and lovs.""IsDeleted""=false and  lovs.""CompanyId""='{_userContext.CompanyId}' and lovs.""Code""='ASSIGNMENT_STATUS_ACTIVE'
left join cms.""N_CoreHR_HRDepartment"" as hd on hd.""Id""=assi.""DepartmentId"" and hd.""IsDeleted""=false and hd.""CompanyId""='{_userContext.CompanyId}'
                 left join cms.""N_CoreHR_HRJob"" as hj on hj.""Id""=assi.""JobId"" and hj.""IsDeleted""=false and hj.""CompanyId""='{_userContext.CompanyId}' 
                  where u.""IsDeleted""=false and u.""CompanyId""='{_userContext.CompanyId}'  #WHERE# ";
...
Lov7.""Name"" as AssignmentTypeName, u.""PhotoId"" as PhotoName,d.""DepartmentName"" as ""OrganizationName"", p.""NtsNoteId"" as ""PersonNoteId""
from cms.""N_CoreHR_HRPerson"" as p
...
select s.""NoteSubject"" as NoteSubject, s.""NoteDescription"" as NoteDescription,
s.""StartDate"" as StartDate,s.""CreatedDate"" as CreatedDate, s.""Id"" as Id, '' as DepartmentId
--,a.""FileId"" as FileId,f.""FileName"" as FileName
from public.""NtsNote"" as s 
join public.""Template"" as t on s.""TemplateId""=t.""Id"" and t.""IsDeleted""=false and t.""CompanyId""='{_userContext.CompanyId}'
join public.""TemplateCategory"" as tc on t.""TemplateCategoryId""=tc.""Id"" and tc.""Code""='ANNOUNCEMENT' and tc.""IsDeleted""=false and tc.""CompanyId""='{_userContext.CompanyId}'
...
left join public.""LOV"" as lov on lov.""Id""=s.""NoteStatusId"" and lov.""IsDeleted""=false and lov.""CompanyId""='{_userContext.CompanyId}'
where ((s.""ExpiryDate"" is not null and s.""StartDate""<= '{DateTime.Now}' and s.""ExpiryDate"">= '{DateTime.Now}') or (s.""ExpiryDate"" is null)) and lov.""Code"" = 'NOTE_STATUS_INPROGRESS' and
(ann.""ReferenceId"" is null ) and s.""IsDeleted""=false and s.""CompanyId""='{_userContext.CompanyId}'
...
left join public.""Company"" as c on gm.""ReferenceTo""=c.""Id"" and c.""IsDeleted""=false and c.""CompanyId""='{_userContext.CompanyId}'
        ...
        public async Task<bool> ValidateUserGuideSequenceNo(PostMessageViewModel model)
        {
            var query = $@"Select * from cms.""N_CoreHR_UserGuide"" where ""SequenceOrder""={model.SequenceOrder} and ""Id""!='{model.UdfNoteTableId}' and ""IsDeleted""=false ";
            var result = await _queryPostMsg.ExecuteQuerySingle(query, null);
            if (result != null)
            {
                return true;
        ...
        public async Task<List<PostMessageViewModel>> GetUserGuideData()
        {
            var query = $@"Select *, n.""Id"" as PostId, cu.""Name"" as CreatedByName from cms.""N_CoreHR_UserGuide"" as gm
                            join public.""NtsNote"" as n on n.""Id"" = gm.""NtsNoteId""
                            join public.""User"" as u on u.""Id"" = n.""CreatedBy""
                            join public.""User"" as cu on cu.""Id"" = gm.""CreatedBy""
                            where n.""IsDeleted""=false";
            var result = await _queryPostMsg.ExecuteQueryList(query, null);
...
where tc.""TemplateType""=5 and tc.""IsDeleted""=false and  tc.""CompanyId""='{_userContext.CompanyId}'";
        ...
        public async Task<UserViewModel> GetManagerData()
        {
            string Query = $@"SELECT u1.*
               FROM public.""User"" as  u
join public.""User"" as u1 on u.""LineManagerId""=u1.""Id"" 
where  u.""IsDeleted""=false and  u.""Id""='{_userContext.UserId}'";
            var result = await _queryRepo1.ExecuteQuerySingle<UserViewModel>(Query, null); //await _queryRepo2.ExecuteQuerySingle(query, null);
            return result;
        }
...
where ""EmployeeId""='{udf.UserId}' ";
        ...
        public async Task<CommandResult<NoteTemplateViewModel>> CreateNewPerson(NoteTemplateViewModel viewModel)
        {
            ...
            else
            {
                ...
                if (userResult.IsSuccess)
                {
                    ...
                    perModel.BusinessHierarchyReferenceType = rowData["BusinessHierarchyReferenceType"].ToString();
                    // };
        ...
        public async Task<IList<NoteViewModel>> GetGroupMessageNew(EndlessScrollingRequest searchModel)
        {
            ...
            var match = $@"With cte as ( select gm.*,n.""Id"" as NoteId,n.""NoteDescription"" as NoteDescription,ou.""Id"" as OwnerUserId,ou.""Name"" as SourceName ,ou.""PhotoId"" as UserPhotoId ,
case when gm.""ReferenceType""='5' then case when  d.""DepartmentName"" is null then c.""Name"" else d.""DepartmentName"" end else case when gm.""ReferenceTo"" is null then 'self' else t.""Name"" end end as TargetName
,case when gm.""ReferenceType""='5' then case when  d.""DepartmentName"" is null then 'COMPANY' else 'ORG' end else case when gm.""ReferenceTo"" is null then 'SELF' else t.""Name"" end end as SourcePost
,'To' as Action,f.""Id"" as FileId, f.""FileExtension""
                            from cms.""N_CoreHR_GroupMessage"" as gm
...
left join public.""Company"" as c on gm.""ReferenceTo""=c.""Id"" and c.""IsDeleted""=false and c.""CompanyId""='{_userContext.CompanyId}'
                            where gm.""IsDeleted""=false and gm.""CompanyId""='{_userContext.CompanyId}') 
				 select cte.*,(select Count(n.*) from  public.""NtsNote"" as n 
where n.""ParentNoteId""=cte.""NtsNoteId"" and n.""OwnerUserId""='{_repo.UserContext.UserId}' and n.""IsDeleted""=false ) as ""ILiked"",
(select Count(n.*) from  public.""NtsNote"" as n 
where n.""ParentNoteId""=cte.""NtsNoteId""  and n.""IsDeleted""=false ) as ""LikesUserCount"",
(with shared as (select n.""Id"" as Id,u.""Name"" as Name,'User' as Type,u.""PhotoId"" as PhotoId
                              from public.""NtsNoteShared"" as n
                               join public.""User"" as u ON u.""Id"" = n.""SharedWithUserId"" and u.""IsDeleted""=false
                              
                              where n.""NtsNoteId""=cte.""NtsNoteId"" AND n.""IsDeleted""= false
union select n.""Id"" as Id,t.""Name"" as Name,'Team' as Type, t.""LogoId"" as PhotoId
                              ...
                              where n.""NtsNoteId""=cte.""NtsNoteId"" AND n.""IsDeleted""= false) select count(*) as ""ShareCount"" from shared),(with commentsCount as (select n.""Id"" as Id,ub.""Name"" as CommentedByUserName,ub.""PhotoId"" as PhotoId,
...
left join public.""NtsNoteComment"" as p on p.""Id""=n.""ParentCommentId"" and p.""IsDeleted""=false
 where n.""NtsNoteId""=cte.""NtsNoteId"" AND n.""IsDeleted""= false and (nut.""Id"" is null or nut.""CommentToUserId""='{_repo.UserContext.UserId}'
or n.""CommentedByUserId""='{_repo.UserContext.UserId}'  )
group by  n.""Id"",ub.""Name"",ub.""PhotoId"",
n.""CommentedDate"",n.""Comment"") select Count(*) as ""CommentsCount"" from commentsCount) from cte
                            ";
        ...
        public async Task<List<AssignmentViewModel>> GetEmployeeDirectory()
        {

            var list = await _hRCoreQueryBusiness.GetEmployeeDirectoryData(); //await _queryRepo2.ExecuteQuerySingle(query, null);
            return list;

        }

        public async Task<List<IdNameViewModel>> GetNonExistingUserInHierarchy(string hierarchyId, string userId)
        {
            string query = $@" select u.""Id"" as Id ,u.""Name"" as Name
                            from public.""User"" as u
join public.""UserPortal"" as up on up.""UserId""=u.""Id"" and up.""IsDeleted""=false
where ((u.""IsDeleted""=false and  u.""CompanyId""='{_userContext.CompanyId}') and up.""PortalId""='{_userContext.PortalId}'  and u.""Id"" not in(SELECT ""RootNodeId"" FROM public.""HierarchyMaster"" 
where ""Id""='{hierarchyId}' and ""IsDeleted""=false and ""CompanyId""='{_userContext.CompanyId}') and u.""Id"" not in 
	  (SELECT ""UserId"" FROM public.""UserHierarchy"" 
where ""IsDeleted""=false and ""CompanyId""='{_userContext.CompanyId}' and ""HierarchyMasterId""='{hierarchyId}') 
--and u.""Id"" not in 	  (SELECT ""ParentUserId"" FROM public.""UserHierarchy"" 
--where ""IsDeleted""=false and ""CompanyId""='{_userContext.CompanyId}' and ""IsDeleted""=false and ""ParentUserId"" is not null and ""HierarchyMasterId""='{hierarchyId}')
) or u.""Id""='{userId}' order by u.""Name""
";
            var list = await _queryRepo1.ExecuteQueryList(query, null);
            return list;
        }
        public async Task<List<IdNameViewModel>> GetExistingUserInHierarchyExceptChild(string hierarchyId, string userId)
        {
            string query = $@" select distinct u.""Id"" as Id ,u.""Name"" as Name
                            from public.""User"" as u 
							join public.""UserHierarchy"" as uh on uh.""UserId""=u.""Id"" and uh.""IsDeleted""=false and ""ParentUserId""<>'' and uh.""HierarchyMasterId""='{hierarchyId}' and uh.""LevelNo""='1'
join public.""UserPortal"" as up on up.""UserId""=u.""Id"" and up.""IsDeleted""=false
where ((u.""IsDeleted""=false and  u.""CompanyId""='{_userContext.CompanyId}') and up.""PortalId""='{_userContext.PortalId}'  and  u.""Id"" not  in 
	  (WITH RECURSIVE ChildUserHierarchy AS(
          select  u.""Name"",u.""Id"",uh.""UserId""
from public.""UserHierarchy"" as uh
join public.""HierarchyMaster"" as hm on hm.""Id""=uh.""HierarchyMasterId"" and hm.""Id""='{hierarchyId}' and  hm.""IsDeleted""=false and hm.""CompanyId""='{_userContext.CompanyId}'
 join public.""User"" as u on u.""Id""=uh.""UserId"" and  u.""IsDeleted""=false and u.""CompanyId""='{_userContext.CompanyId}' 
	 where uh.""ParentUserId""='{userId}' and uh.""LevelNo""=1 and uh.""OptionNo""=1 and  uh.""IsDeleted""=false and uh.""CompanyId""='{_userContext.CompanyId}'
...
join public.""HierarchyMaster"" as hm on hm.""Id""=uh.""HierarchyMasterId"" and hm.""Id""='{hierarchyId}' and  hm.""IsDeleted""=false and hm.""CompanyId""='{_userContext.CompanyId}'
	 ...
	 where  uh.""LevelNo""=1 and uh.""OptionNo""=1       and  uh.""IsDeleted""=false and uh.""CompanyId""='{_userContext.CompanyId}'          )
                            select ""UserId"" from ChildUserHierarchy
) 
...
";
        ...
        {
            var list = await _hRCoreQueryBusiness.GetEmployeeTransferItemList(employeeId); //await _queryRepo2.ExecuteQuerySingle(query, null);
            return list;
        }
        public async Task<List<EmployeeTransferItemViewModel>> GeEmployeeFormTransferItemList(string employeeId)
        {
            var lov = await _lovBusiness.GetList(x => x.LOVType == "Item_Transfer_Type" && x.Code == "TRANSFER");
            var list = await _hRCoreQueryBusiness.GeEmployeeFormTransferItemList(employeeId, lov.FirstOrDefault().Id); //await _queryRepo2.ExecuteQuerySingle(query, null);
            return list;
        }
    }
}
