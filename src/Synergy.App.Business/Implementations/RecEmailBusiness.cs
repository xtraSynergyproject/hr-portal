...
namespace Synergy.App.Business
{
    public class RecEmailBusiness : BusinessBase<EmailViewModel, Email>, IRecEmailBusiness
    {
        ...
        public async override Task<CommandResult<EmailViewModel>> Create(EmailViewModel model, bool autoCommit = true)
        {
        ...
        public async override Task<CommandResult<EmailViewModel>> Edit(EmailViewModel model, bool autoCommit = true)
        {
        ...
        private async Task SetEnvironment(MailMessage mailMessage, EmailViewModel email)
        {
            ...
            if (env == "DEV")
            {
                ...
                var recipient = await _repo.GetSingle<CompanySettingViewModel, CompanySetting>(x => x.Code == "TEST_EMAIL_RECIPIENTS");
                if (recipient != null && recipient.Value.IsNotNullAndNotEmpty())
                {
                    testRecipients = recipient.Value;
                }
