...
namespace Synergy.App.Business
{
    public class BusinessRuleBusiness : BusinessBase<BusinessRuleViewModel, BusinessRule>, IBusinessRuleBusiness
    {
        ...
        private readonly IBreQueryBusiness _breQueryBusiness;
        public BusinessRuleBusiness(IRepositoryBase<BusinessRuleViewModel, BusinessRule> repo
            ...
            IBreQueryBusiness breQueryBusiness,
             IServiceProvider serviceProvider) : base(repo, autoMapper)
        {
            ...
            _breQueryBusiness = breQueryBusiness;
        }
        public async override Task<CommandResult<BusinessRuleViewModel>> Create(BusinessRuleViewModel model, bool autoCommit = true)
        {
            ...
            if (result.IsSuccess)
            {
                ...
                await _businessRuleNodeBusiness.Create(brnModel, autoCommit);
            }

            return CommandResult<BusinessRuleViewModel>.Instance(model, result.IsSuccess, result.Messages);
        }
        public async override Task<CommandResult<BusinessRuleViewModel>> Edit(BusinessRuleViewModel model, bool autoCommit = true)
        {
            var validateName = await IsExists(model);
            if (!validateName.IsSuccess)
            {
...
false#end";
        ...
        {
            var result = await _breQueryBusiness.GetBusinessRuleActionListData(templateId, actionType);
            return result;
