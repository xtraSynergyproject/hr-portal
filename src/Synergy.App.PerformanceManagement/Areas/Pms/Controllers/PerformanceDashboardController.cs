...
namespace CMS.UI.Web.Areas.Pms.Controllers
{
    ...
    public class PerformanceDashboardController : ApplicationController
    {
        ...
        public async Task<ActionResult> Index(string pageName, string ProjectId,string stageId,string year,string PerformanceUser)
        {
            var userId = _userContext.UserId;
            var isProjectManager = IsProjectManager();
            ...
            ServiceViewModel model = new ServiceViewModel()
            {
                ...
                PerformanceUser= PerformanceUser
            };
        ...
        public async Task<ActionResult> PerformanceDetailsBanner(string ProjectId, string PerformanceStage, string pageName, string PerformanceUser, string PerformanceUserYear, string type, string departmentId = null)
        {
            ...
            else
            {
                model = new ServiceViewModel()
                {
                    ...
                    PageName = pageName,
                    PerformanceUser = PerformanceUser,
                    PerformanceUserYear = PerformanceUserYear,
        ...
        public async Task<ActionResult> PerformanceDashboard(string PerformanceId, string stageId = null, string performanceUserId = null)
        {
            ...
            if (model == null)
            {
                model = new PerformanceDashboardViewModel();
            }
            return View(model);
        }
        ...
        public async Task<ActionResult> PerformanceResult(string pageName, string ProjectId,string stageId=null, string departmentId = null,string userId=null,string year=null)
        {
            ...
            {
                model.Id = ProjectId;
                model.PerformanceUser = userId.IsNullOrEmpty()?_userContext.UserId:userId;
                model.PerformanceUserYear = year.IsNullOrEmpty()? DateTime.Now.Year.ToString() : year;

                if (stageId.IsNullOrEmpty())
                {
                    var data = await _performanceManagementBusiness.GetPerDocMasterStageDataByServiceId(ProjectId,userId);
                    if (data.Count > 0)
                    {
                        model.PerformanceStage = data.FirstOrDefault().StageId;
                    }
                ...
                {
                    model.PerformanceStage = stageId;
                }                
            }           
