    ...
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IHttpContextAccessor accessor)
        {
           ...
           // }
            ...
            try
            {
                RecurringJob.AddOrUpdate<Synergy.App.Business.HangfireScheduler>(x => x.UpdateNtsStatus(), "0 */4 * * *");
            }
            catch (Exception)
            {
            ...
            try
            {
                RecurringJob.AddOrUpdate<Synergy.App.Business.HangfireScheduler>(x => x.UpdateReceiveEmailForProjectManagement(), Cron.MinuteInterval(5));
            }
            catch (Exception)
            {
            ...
            try
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    RecurringJob.AddOrUpdate<Synergy.App.Business.HangfireScheduler>(x => x.ConvertFileToPdf(), Cron.MinuteInterval(5));
                }
            }
            catch (Exception)
            {
            ...
            try
            {
                RecurringJob.AddOrUpdate<Synergy.App.Business.HangfireScheduler>(x => x.DailyJob(), Cron.Daily());
            }
            catch (Exception)
            {
            ...
            try
            {
                BackgroundJob.Enqueue<Synergy.App.Business.HangfireScheduler>(x => x.CreateHangfireJobs());
            }
            catch (Exception)
            {
            ...
            var defaultTusConfig = new DefaultTusConfiguration
            {
                ...
                Events = new Events
                {
                    ...
                    OnFileCompleteAsync = async ctx =>
                    {
                        ...
                        try
                        {
                            var isSingleUploadMetaData = metadatas["isSingleUpload"];
                            if (isSingleUploadMetaData != null)
                            {
                                var isSingleUploadStr = isSingleUploadMetaData.GetString(Encoding.UTF8);
                                if (isSingleUploadStr != null || isSingleUploadStr != String.Empty)
                                {
                                    isSingleUpload = bool.Parse(isSingleUploadStr);
                                }
                        ...
                        using (var serviceScope = _app.ApplicationServices.CreateScope())
                        {
                            ...
                            var result = await _documentBusiness.CreateDocumentByUpload(newpath, fileName, extensionName, parentId, userId, templateId, bacthId, isSingleUpload);
                        }
                        ////Create file end
                    },
