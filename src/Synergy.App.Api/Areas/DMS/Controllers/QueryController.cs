...
namespace Synergy.App.Api.Areas.DMS.Controllers
{
    ...
    public class QueryController : ApiController
    {
        ...
        public QueryController(AuthSignInManager<ApplicationIdentityUser> customUserManager, IUserBusiness userBusiness,
        ...
        [HttpGet]
        [Route("GetDMSTree")]
        public async Task<object> GetDMSTree(string UserID)
        {
            var workspaces = await _documentBusiness.GetWorkspacebyUser(UserID);
            var json = JsonConvert.SerializeObject(workspaces);
            return json;
        }

        [HttpGet]
        [Route("GetDMSDocument")]
        public async Task<object> GetDMSDocumentbyUser(string UserID)
        {
            var workspaces = await _documentBusiness.GetDocumentbyUser(UserID);
            var json = JsonConvert.SerializeObject(workspaces);
            return json;
        }
        ...
        [HttpGet]
        [Route("CreateFolder")]
        public async Task<object> CreateFolder(string UserID, string FolderName, string ParentId)
        {
            await Authenticate(UserID);
            var foldername = await _documentBusiness.CreateFolder(FolderName, ParentId);
            var json = JsonConvert.SerializeObject(foldername);
            return json;
        }
        [HttpGet]
        [Route("Getfolderpath")]
        ...
        public async Task<ActionResult> ReadDocumentData(string templateId, string projectNo, string noteNo, string docDescription, int docCount = 0)
        {
            ...
            //{
            //    list = list.Where(x => get.NoteNo.ToLower().Contains(noteNo.ToLower())).ToList();
            //}
        ...
        [Route("DmsUploadFiles/")]
        [HttpPost]
        public async Task<IActionResult> DMSUploadFiles(string type)
        {
            //Fetch the File.
            IFormFile postedFile = Request.Form.Files[0];
            var fileBytes = new byte[postedFile.Length];
            var fileId = "0";
            ...
            if (res.IsSuccess)
            {
                var user = await _userBusiness.GetSingle(x => x.Name == "Smart City Admin");
                if (user.IsNotNull())
                {
                    userId = user.Id;
                }
                ...
                {
                    var workspace1 = await _noteBusiness.GetList(x => x.NoteSubject == "Jammu Smart City Workspace");
                    var workspace = workspace1.First();
                    if (workspace.IsNotNull())
                    {
                        var notefolder = await _noteBusiness.GetSingle(x => x.NoteSubject == "JMC_OBPS" && x.ParentNoteId == workspace.Id);
                        if (notefolder.IsNotNull())
                        {
                        ...
                        {
                            var templateModel = new NoteTemplateViewModel();
                            templateModel.ActiveUserId = userId;
                            templateModel.DataAction = DataActionEnum.Create;
                            templateModel.TemplateCode = "GENERAL_FOLDER";
                            var ntmodel = await _noteBusiness.GetNoteDetails(templateModel);
                            ntmodel.NoteSubject = "JMC_OBPS";
                            ntmodel.NoteStatusCode = "NOTE_STATUS_INPROGRESS";
                            ntmodel.ParentNoteId = workspace.Id;
                            ntmodel.OwnerUserId = userId;
                            dynamic folderexo = new System.Dynamic.ExpandoObject();
                            ((IDictionary<String, Object>)folderexo).Add("Code", "JMC_OBPS");
                            ntmodel.Json = Newtonsoft.Json.JsonConvert.SerializeObject(folderexo);
                            var result = await _noteBusiness.ManageNote(ntmodel);
                            if (result.IsSuccess)
                            {
                                fileId = result.Item.NoteId; 
                            }
                        }
                ...
                if(folderId.IsNotNullAndNotEmpty() && userId.IsNotNullAndNotEmpty() && type.IsNotNullAndNotEmpty())
                {
                    var model = new NoteTemplateViewModel
                    {
                        TemplateCode = "JMC_OBPS",
                    ...
                    {
                        var val = Request.Form[item];
                        if (val.Count > 0)
                        {
                            ((IDictionary<String, Object>)exo).Add(item, val[0]);
                        }
                       
                    }
                    var byName = (IDictionary<string, object>)exo;
                    var newmodel = await _noteBusiness.GetNoteDetails(model);
                    newmodel.NoteSubject = (string)byName["AuthorizationId"];
                    ...
                    {
                        fileId = noteResult.Item.NoteId;
                    }
                }

                else if (folderId.IsNotNullAndNotEmpty() && userId.IsNotNullAndNotEmpty())
                {
                    var model = new NoteTemplateViewModel
                    {
                    ...
                    newmodel.NoteStatusCode = "NOTE_STATUS_DRAFT";
                    newmodel.Json = "{}";
                    dynamic fexo = new System.Dynamic.ExpandoObject();
                    if (res.Item.Id.IsNotNullAndNotEmpty())
                    {

                        ((IDictionary<String, Object>)fexo).Add("fileAttachment", res.Item.Id);

                        newmodel.Json = Newtonsoft.Json.JsonConvert.SerializeObject(fexo);
                    }
                    var noteResult = await _noteBusiness.ManageNote(newmodel);
                    if (noteResult.IsSuccess)
                    {
