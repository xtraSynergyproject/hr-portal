...
//else
var server = "wss://10.100.100.101/ws";
...
function startRtspStream(rtsp, streaming, j) {
	debugger;
	let streamId = Math.floor(Math.random() * (1000000 - 10 + 1)) + 0;
	Janus.log(streamId);
	Janus.log("---- Starting RTSP Stream: " + rtsp + " with ID: " + streamId);
	streaming.send(
		{
			"message": {
				"request": "create",
				"type": "rtsp",
				"id": streamId,
				"description": "RTSP Test 1" + streamId,
				"video": true,
				"audio": false,
			...
			//var body = { "request": "watch", id: parseInt(response.created) };
			//streaming.send({ "message": body });
			//// No remote video yet
			//$('#stream-' + j).append('<video class="rounded centered" id="waitingvideo-' + j + '" width=50% height=50% />');
			//if (spinner == null) {
			//	//var target = document.getElementById('stream-1' + j);
			//	var target = document.getElementById('stream-1');
