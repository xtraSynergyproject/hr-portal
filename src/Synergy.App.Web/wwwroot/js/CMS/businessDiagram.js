...
this.paper.on('element:contextmenu', (node, x, y) => {
    ...
    if (node.model.attributes.attrs.type == 'TEMPLATE') {
    ...
    } else if (node.model.attributes.attrs.type == 'PRE_LOAD_ROOT') {
        $("#menulist").append("<li class='menu-option' onclick='onEditPreLoadBusinessRule(\"" + selectedTemplateId + "\",\"" + selectedTemplateType + "\");'>Create Business Rule</li>"); //Business Rule
        ...
        if (node.model.attributes.attrs.parentIds[0].includes('PRE_LOAD')) {
            $("#menulist").append("<li class='menu-option' onclick='onCreateProcess(\"" + selectedTemplateId + "\",\"" + node.model.attributes.attrs.dataModel.Id + "\",\"" + node.model.attributes.attrs.dataModel.ReferenceId + "\",false ,\" PreLoad \");'>Add Process</li>");
        }
        else {
        ...
        $("#menulist").append("<li class='menu-option' onclick='onCreateDecision(\"" + selectedTemplateId + "\",\"" + node.model.attributes.attrs.dataModel.Id + "\",\"" + node.model.attributes.attrs.dataModel.ReferenceId + "\");'>Add Decision</li>");
        if (node.model.attributes.attrs.parentIds[0].includes('PRE_LOAD')) {
            $("#menulist").append("<li class='menu-option' onclick='onCreateProcess(\"" + selectedTemplateId + "\",\"" + node.model.attributes.attrs.dataModel.Id + "\",\"" + node.model.attributes.attrs.dataModel.ReferenceId + "\",true,\" PreLoad \");'>Edit Process</li>");
        } else {
...
function onEditPreLoadBusinessRule(templateId, temptype, ruleId) {
    var win = GetMainWindow();
    win.iframeOpenUrl = '/Bre/BusinessRule/CreateBusinessRule?templateId=' + templateId + '&templateType=' + temptype + '&type=PreLoad' + '&businessRuleId=' + ruleId;
    win.OpenWindow({ Title: 'Business Rule', Width: 500, Height: 600 });
    return false;
}
...
function onCreateProcess(tempid, rulenodeid, ruleid, edit, nodeType) {
    debugger;
    //alert("Process");
    ...
    var win = GetMainWindow();
    if (edit == true) {
        win.iframeOpenUrl = '/Bre/Bre/BreResultViewDetails?businessRuleNodeId=' + rulenodeid + '&templateId=' + tempid + "&ruleId=" + ruleid + "&isBusinessDiagram=true&nodeType=" + nodeType;
        win.OpenWindow({ Title: 'Process', Width: 800, Height: 600 });
    }
    else {
        win.iframeOpenUrl = '/Bre/Bre/BreResultViewDetails?parentId=' + rulenodeid + '&templateId=' + tempid + "&ruleId=" + ruleid + "&isBusinessDiagram=true&nodeType=" + nodeType;
        win.OpenWindow({ Title: 'Process', Width: 800, Height: 600 });
    }
