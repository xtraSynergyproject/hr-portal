...
namespace CMS.UI.Web.Areas.CMS.Controllers
{
...

    {
        ...
        public async Task<JsonResult> GetTemplateCategoryListByPortal(string portalId)
        {
            var data = await _templatecategoryBusiness.GetList(x => x.PortalId == portalId);
            return Json(data);
        }
        public async Task<JsonResult> GetCategoryListByType(TemplateTypeEnum templateType)
        {
            var data = await _templatecategoryBusiness.GetList(x => x.TemplateType == templateType);
            return Json(data);
        }
        ...
        public async Task<IActionResult> Manage(TemplateCategoryViewModel model)
        {
            if (ModelState.IsValid)
            {
                if (model.DataAction == DataActionEnum.Create)
                {
                    ...
                    if (result.IsSuccess)
                    {    
                        string[] templateTypes = { "Page","Form", "Note", "Task", "Service", "Custom" };
                        List<string> list = new List<string>(templateTypes);
                        list.Remove(model.TemplateType.ToString());
                        if (!model.CopyInPage)
                        {
                            list.Remove("Page");
                        }
                        ...
                        if (!model.CopyInService)
                        {
                            list.Remove("Service");
                        }
                        if (!model.CopyInCustom)
                        {
                            list.Remove("Custom");
                        }
                        foreach (var type in list)
                        {
                            TemplateTypeEnum temtype = (TemplateTypeEnum)Enum.Parse(typeof(TemplateTypeEnum), type);
                            var exist = await _templatecategoryBusiness.GetSingle(x => x.Code == data.Code && x.TemplateType == temtype);
                            if (!exist.IsNotNull())
                            {
                                string parentId=null;
                                if (data.ParentId.IsNotNullAndNotEmpty())
                                {
                                    var parent = await _templatecategoryBusiness.GetSingleById(data.ParentId);
                                    var newparent = await _templatecategoryBusiness.GetSingle(x => x.Code == parent.Code && x.TemplateType == temtype);
                                ...
                                model.TemplateType = temtype;
                                model.ParentId = parentId;
                                await _templatecategoryBusiness.Create(model);
                            }
                        }
                    ...
                    {
                        return Json(new { success = false, error = result.HtmlError.ToString() });
                    }
                }
