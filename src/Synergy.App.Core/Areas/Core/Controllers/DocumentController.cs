...
namespace CMS.UI.Web.Controllers
{
    ...
    public class DocumentController : ApplicationController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> SaveBlsAppointment(IList<IFormFile> file, string appointmentId)
        {
            try
            {
                foreach (var f in file)
                {
                    ...
                    if (result.IsSuccess)
                    {
                        Response.Headers.Add("fileId", result.Item.Id);
                        Response.Headers.Add("fileName", result.Item.FileName);
                        var appointment = await _fileBusiness.GetSingleById<BLSAppointmentViewModel, BLSAppointment>(appointmentId);
                        if (appointment != null)
                        {
                            appointment.ApplicantPhotoId = result.Item.Id;
                            await _fileBusiness.Edit<BLSAppointmentViewModel, BLSAppointment>(appointment);
                        }
                        return Json(new { success = true, fileId = result.Item.Id, filename = result.Item.FileName });
                    }
                    else
