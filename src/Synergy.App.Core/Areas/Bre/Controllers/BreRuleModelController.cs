...
namespace CMS.UI.Web.Controllers
{
    ...
    public class BreRuleModelController : ApplicationController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> ManageBusinessRuleForStepTask(BusinessRuleModelViewModel model)
        {
            if (ModelState.IsValid)
            {
                var left = "";
                ...
                var symbol = "";
                var rightB = "";
                var values = JsonConvert.DeserializeObject<Dictionary<string, string>>(model.DataJson);
                foreach (var item in values)
                {

                    left = $"Input.Data.{item.Key}";
                    leftB = $"'$!Input.Data.{item.Key}'";
                    symbol = "==";
                    rightB = right = $"'{item.Value}'";
                    model.OperationValue = $"{model.OperationValue} {left} {symbol} {right} &&";
                    model.OperationBackendValue = $"{model.OperationBackendValue}{leftB} {symbol} {rightB}  &&";
                }
                if (model.OperationValue.IsNotNullAndNotEmpty())
                {
                    model.OperationValue = model.OperationValue.TrimEnd('&').TrimEnd('&');
                }
                if (model.OperationBackendValue.IsNotNullAndNotEmpty())
                {
                    model.OperationBackendValue = model.OperationBackendValue.TrimEnd('&').TrimEnd('&');
                }
                model.BusinessRuleType = BusinessRuleTypeEnum.Operational;
                if (model.DataAction == DataActionEnum.Create)
                {
