    ...
    public class HRDirectController : ApplicationController
    {
        ...
        public HRDirectController(IHRCoreBusiness hRCoreBusiness, IUserContext userContext, ITemplateCategoryBusiness templateCategoryBusiness
            ...
            , IUserHierarchyBusiness userHierarchyBusiness, IHierarchyMasterBusiness hierarchyMasterBusiness,
              IUserBusiness userBusiness, IPayrollBatchBusiness payrollBatchBusiness, IPortalBusiness portalBusiness,
        ...
        public async Task<IActionResult> HRDirect(string pName = null)
        {
            var portal = await _cmsBusiness.GetSingleById<PortalViewModel, Portal>(_userContext.PortalId);
            var menus = await _portalBusiness.GetMenuItems(portal, _userContext.UserId, _userContext.CompanyId, "HR_DIRECT");
        ...
        public async Task<IActionResult> HRCore(string pName = null)
        {
            var portal = await _cmsBusiness.GetSingleById<PortalViewModel, Portal>(_userContext.PortalId);
            var menus = await _portalBusiness.GetMenuItems(portal, _userContext.UserId, _userContext.CompanyId, "HR_CORE");
            if (pName.IsNullOrEmpty())
            {
                var mg = menus.Where(x => x.PageType == TemplateTypeEnum.MenuGroup).FirstOrDefault();
                var page = menus.Where(x => x.PageType != TemplateTypeEnum.MenuGroup && x.ParentId == mg.Id).OrderBy(x => x.SequenceOrder).FirstOrDefault();
        ...
        public async Task<IActionResult> HRReports(string pName = null)
        {
            var portal = await _cmsBusiness.GetSingleById<PortalViewModel, Portal>(_userContext.PortalId);
            var menus = await _portalBusiness.GetMenuItems(portal, _userContext.UserId, _userContext.CompanyId, "HR_REPORTS");
            if (pName.IsNullOrEmpty())
            {
                var mg = menus.Where(x => x.PageType == TemplateTypeEnum.MenuGroup).FirstOrDefault();
                var page = menus.Where(x => x.PageType != TemplateTypeEnum.MenuGroup && x.ParentId == mg.Id).OrderBy(x => x.SequenceOrder).FirstOrDefault();
        ...
        {
            var model = new HRDirectPageViewModel();
            return View(model);
        }

        [HttpPost]
        public async Task<IActionResult> ManageHRDirect(HRDirectPageViewModel model)
        {
        ...
        public async Task<ActionResult> GetPersonValueMapper(string value, string filters)
        {
            long dataItemIndex = -1;

            if (value != null)
            {
                var list = await _hRCoreBusiness.GetPersonFilterdList(filters, 0, 0, value);
        ...
        {
            var list = await _hRCoreBusiness.GetPersonFilterdList(filters, pageSize, page, null);
        ...
        public async Task<ActionResult> GetSwitchPersonValueMapper(string legalEntityId, string value, string filters)
        {
            long dataItemIndex = -1;

            if (value != null)
            {
                var list = await _hRCoreBusiness.GetPersonAssignmentDetails(filters, 0, 0, value, legalEntityId);
                //await _userBusiness.GetSwitchUserList(null, null, false, filters, 0, 0);
                dataItemIndex = list.ItemIndex;
            }
            return Json(dataItemIndex);
        }
        public async Task<ActionResult> GetSwitchToPersonVirtualData(string legalEntityId, int page, int pageSize, string filters, string hierarchyId, string parentId = null)
        {
            var list = await _hRCoreBusiness.GetPersonAssignmentDetails(filters, pageSize, page, null, legalEntityId);
            //await _userBusiness.GetSwitchUserList(_userContext.UserId, _userContext.LoggedInAsByUserId, hasLoggedinAsPermission, filters, pageSize, page);
        ...
        public IActionResult Transfer(string EmpId)
        {
            var model = new MisconductViewModel();
            if (EmpId.IsNotNullAndNotEmpty())
            {
        ...
        public async Task<ActionResult> GetTransferGridData(string Id = null, string UserId = null)
        {
            var userid = "";
            ...
            var model = await _hRCoreBusiness.GetTransferDetails(userid);
        ...
        public async Task<ActionResult> TerminatePerson(string personId = null, string layoutMode = null)
        {
            ...
            //{
            //    ViewBag.Layout = "~/Areas/Core/Views/Shared/_LayoutPopup.cshtml";
            //}
            return View(vm);
        }
        [HttpPost]
        ...
        public async Task<IActionResult> EmployeeProfileIndex(string personId = null)
        {
            var person = await _hRCoreBusiness.GetPersonDetailsById(personId);

            var persondetail = await _hRCoreBusiness.GetAssignmentDetails(personId, null);
            AssignmentViewModel model = persondetail.FirstOrDefault();
            model.PersonId = personId;

            ViewBag.PersonId = personId;
            if (person.IsNotNull())
            {
                ViewBag.UserId = person.UserId;
                ViewBag.SalaryInfoId = model.SalaryInfoId;
            }

            return View(model);
        }

        public async Task<IActionResult> Assignment(string personId)
        {
            var userrole = _userContext.UserRoleCodes.IsNullOrEmpty() ? new string[] { } : _userContext.UserRoleCodes.Split(",");
        ...
        public async Task<IActionResult> Contract(string personId)
        {
            var userrole = _userContext.UserRoleCodes.IsNullOrEmpty() ? new string[] { } : _userContext.UserRoleCodes.Split(",");
        ...
        public async Task<IActionResult> Payroll(string salaryInfoId, string personId)
        {
            var userrole = _userContext.UserRoleCodes.IsNullOrEmpty() ? new string[] { } : _userContext.UserRoleCodes.Split(",");
        ...
        public async Task<ActionResult> EmployeeDirectory()
        {
        ...
        public async Task<ActionResult> GetEmployeeDirectory()
        {
            ...
            var model = await _hRCoreBusiness.GetEmployeeDirectory();
            return Json(model);
