...
namespace Synergy.App.Business
{
    public class TemplateBusiness : BusinessBase<TemplateViewModel, Template>, ITemplateBusiness
    {
        ...
        public TemplateBusiness(IRepositoryBase<TemplateViewModel, Template> repo, IMapper autoMapper,
            ...
            IFormTemplateBusiness formTemplateBusiness, ICmsQueryBusiness cmsQueryBusiness,
        ...
        private async Task<CommandResult<TemplateViewModel>> IsNameExists(TemplateViewModel viewModel)
        {
            ...
            var givenName = await _cmsQueryBusiness.SelectTemplateByName(viewModel);
            ...
            var code = await _cmsQueryBusiness.SelectTemplateByCode(viewModel);
            ...
            if (result.IsSuccess)
            {
                switch (model.TemplateType)
                {
                    ...
                    case TemplateTypeEnum.Form:
                        var formResult = await _formTemplateBusiness.Create(new FormTemplateViewModel { TemplateId = result.Item.Id, IsChildTable = model.IsChildTable });
        ...
        {
            var result = await _cmsQueryBusiness.CheckTemplate(model);
            return result;
        }
        ...
        {
            var list = await _cmsQueryBusiness.GetNoteTemplateList(tCode, tcCode, mCodes, templateIds, categoryIds, categoryType, portalNames);
        ...
        {
            var list = await _cmsQueryBusiness.GetTemplateList(tCode, tcCode, mCodes, templateIds, categoryIds, categoryType, portalNames);
            //var taskList = list.Where(x => x.TemplateType == TemplateTypeEnum.Task && x.TaskType != TaskTypeEnum.StepTask);
            return list;
        }
        public async Task<List<TemplateViewModel>> GetAdhocTemplateList(string tCode, string tcCode, string moduleId)
        {
            var list = await _cmsQueryBusiness.GetAdhocTemplateList(tCode, tcCode, moduleId);
            //var taskList = list.Where(x => x.TemplateType == TemplateTypeEnum.Task && x.TaskType != TaskTypeEnum.StepTask);
            return list;
        }
        ...
        {
            var list = await _cmsQueryBusiness.GetTemplateServiceList(tCode, tcCode, mCodes, templateIds, categoryIds, categoryType, allBooks, portalNames, serviceType, groupCodes);
            if (allBooks)
            {
        ...
        {
            var list = await _cmsQueryBusiness.GetTemplateListByTaskTemplate(taskTemplateId);
            return list;
        }
        ...
        public async Task<List<BusinessDiagramNodeViewModel>> GetTemplateBusinessDiagram(string templateId)
        {
            ...
            var list = await _cmsQueryBusiness.GetTemplateBusinessDiagram(templateId);
        ...
        {
            var result = await _cmsQueryBusiness.GetComponentsList(templateId);
            return result;
        }
        ...
        public async Task<List<TemplateViewModel>> GetAllowedTemplateList(string categoryCode, string userId, TemplateTypeEnum? templateType, TaskTypeEnum? taskType, string portalCode = null)
        {
            ...
            return await _cmsQueryBusiness.GetAllowedTemplateList(categoryCode, userId, templateType, taskType, portalCode);
        }

        public async Task<List<WorkflowViewModel>> GetWorkFlowDiagramDetails(string id)
        {

            var list = await _cmsQueryBusiness.GetServiceDetail(id);



            var step = await _cmsQueryBusiness.GetWorkflowDiagramDetails(id);
        ...
        public async Task<List<WorkflowViewModel>> GetWorkFlowDiagramDetailsByTemplate(string id)
        {
            ...
            var list = await _cmsQueryBusiness.GetTemplateDetail(id);
            ...
            var step = await _cmsQueryBusiness.GetWorkflowDiagramDetailsByTemplate(id);
            list.AddRange(step);
            var stage = list.Select(x => x.StageName).Distinct();
            if (stage == null || stage.Count() == 1)
            {
                var root = await _repo.GetSingleById(id);

                if (root != null)
                {
                    var rootServiceTemplate = await _repo.GetSingle<ServiceTemplateViewModel, ServiceTemplate>(x => x.TemplateId == root.Id);
                    if (rootServiceTemplate != null)
                    {
                        var templates = await _repo.GetList<ServiceTemplateViewModel, ServiceTemplate>(x => x.RootServiceCode == root.Code);
                        if (templates != null && templates.Any())
                        {
                            list = new List<WorkflowViewModel>();
                            var parentId = root.Id;
                            list.Add(new WorkflowViewModel
                            {
                                StageName = root.DisplayName.Coalesce(root.Name),
                                StepName = "Step 1",
                                Subject = root.DisplayName.Coalesce(root.Name),
                                TemplateId = root.Id,
                                StageId = root.Id,
                                Id = root.Id,
                                Type = "Service",
                            ...
                            var stepTasks = await _stepTaskComponentBusiness.GetStepTaskParentList(root.Id);
                            foreach (var st in stepTasks)
                            {
                                parentId = st.Id;
                                var t = await _repo.GetSingleById(st.Id);
                                list.Add(new WorkflowViewModel
                                {
                                    Id = st.Id,
                                    Subject = root.DisplayName.Coalesce(root.Name),
                                    TemplateId = root.Id,
                                    StageName = t.DisplayName.Coalesce(t.Name),
                                    StageId = t.Id,
                                    StepName = "Step 1",
                                    ParentId = st.Code.Coalesce(root.Id),
                                    Type = "Task",
                                });
                            }
                            foreach (var item in templates)
                            {
                                stepTasks = await _stepTaskComponentBusiness.GetStepTaskParentList(item.TemplateId);
                                var t = await _repo.GetSingleById(item.TemplateId);
                                foreach (var st in stepTasks)
                                {
                                    list.Add(new WorkflowViewModel
                                    {
                    ...
                    //list = await _cmsQueryBusiness.GetTemplateDetail(id);
                }
        ...
        public async Task<List<TemplateViewModel>> GetTemplateServiceListbyTeam(string tCode, string teamId)
        {
            ...
            var list = await _cmsQueryBusiness.GetTemplateServiceListbyTeam(tCode, teamId);
            return list;
        }
        ...
        public async Task<List<TemplateViewModel>> GetTemplateDeleteList()
        {
            ...
            var list = await _cmsQueryBusiness.GetTemplateDeleteList();
            return list;
        }
        public async Task<bool> SetOcrTemplateFileId(string templateId, string fileId)
        {
            await _cmsQueryBusiness.SetOcrTemplateFileId(templateId, fileId);
            return true;
        }

        public async Task<List<TemplateViewModel>> GetMasterList(string groupCode)
        {
            var data = await _cmsQueryBusiness.GetMasterList(groupCode);

            var result = new List<TemplateViewModel>();

            foreach (var cat in data.GroupBy(x => x.TemplateCategoryId))
            {
                var model = new List<TemplateViewModel>();

                foreach (var temp in data)
                {
                    if (cat.Any(x => x.TemplateCategoryId == temp.TemplateCategoryId))
                    {
                ...
                {
                    TemplateCategoryId = cat.Key,
                    TemplateCategoryName = cat.FirstOrDefault().TemplateCategoryName,
                    TemplatesList = model
                }
                );
            }
        ...
        {
            var data = await _cmsQueryBusiness.GetTemplatesList(portalId, templateType);
            ...
            foreach (var cat in data.GroupBy(x => x.TemplateCategoryId))
            {
                ...
                foreach (var temp in data)
                {
                    if (temp.Id.IsNotNull() && cat.Any(x => x.TemplateCategoryId == temp.TemplateCategoryId))
                    {
                        model.Add(new TemplateViewModel()
                        {
                            DisplayName = temp.DisplayName,
        ...
        {
            var data = await _cmsQueryBusiness.GetForeignKeyId(col,val);
            return data;
        }
    }
}
