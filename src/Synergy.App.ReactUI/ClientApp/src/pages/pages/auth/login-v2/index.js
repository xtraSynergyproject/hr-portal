...
const LoginV2 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const api = await axios.post(`https://demo.aitalkx.com/api_gateway/api/generatetoken?username=${email}&password=${password}`)

    // console.log("api response", api.data)

    localStorage.setItem("Authorization", `Bearer ${api.data}`)
    ...
    // window.alert("logged in sucessful")
  }

  const handleChange = (e) => {
    const { name, value } = e.target
    // console.log("object", value, "name", name);
