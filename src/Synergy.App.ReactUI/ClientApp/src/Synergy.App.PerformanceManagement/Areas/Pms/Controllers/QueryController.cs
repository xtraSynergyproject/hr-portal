...
namespace CMS.Web.Areas.Pms.Controllers
{
    ...
    public class QueryController : Controller
    {
        ...
        public async Task<IActionResult> GetUserPerformanceDocumentInfo(string userId, string PerformanceId,string masterStageId)
        {
            ...
            //PerformanceId = "9cae7a6b-32f1-4476-a040-7100f264d4d2";
            //masterStageId = "9c4091fe-7086-4405-bf5f-5aed08b697d8";

            ////Rishi
            //userId = "6268fb19701a2f6858aa09fc";
            //PerformanceId = "6822e3e4-28ff-4795-9698-77618c6c7553";
            //masterStageId = "1de1afa5-3f50-4fa2-99ae-c23fbea3a877";
            ...
            try
            {
                var performanceDoc = await _hRCoreBusiness.GetUserPerformanceDocumentInfo(userId, PerformanceId,masterStageId);
                ...
                if (performanceDoc != null)
                {
                    ...
                    if (performanceDoc.FinalScore.IsNotNullAndNotEmpty())
                    {
                        var finalscorefile = $"PERFORMANCE_REPORT_{performanceDoc.FinalScore}";
                        var finalscore = await _applicationDocumentBusiness.GetSingle(x => x.Code == finalscorefile);
                        if (finalscore.IsNotNull())
                        {
                            var finalscorebytes = await _fileBusiness.GetFileByte(finalscore.DocumentId);
                            if (finalscorebytes.Length > 0)
                            {
                                performanceDoc.FinalScoreImage = Convert.ToBase64String(finalscorebytes, 0, finalscorebytes.Length);
                            }
                        }
                    }
                    else
                    {
                        var finalscorefile = $"PERFORMANCE_REPORT_1";
                        var finalscore = await _applicationDocumentBusiness.GetSingle(x => x.Code == finalscorefile);
                        if (finalscore.IsNotNull())
                        {
                            var finalscorebytes = await _fileBusiness.GetFileByte(finalscore.DocumentId);
                    ...
                    performanceDoc.CurrentDateText = string.Format("{0:dd MMM yyyy}", DateTime.Today);
                }
