    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                try
                {
                    // var redisConnUrl = Configuration.GetConnectionString("RedisConnectionUrl");
                    var redis = ConnectionMultiplexer.Connect(Configuration.GetConnectionString("RedisConnectionUrl"));
                    services.AddDataProtection().SetApplicationName("synergyapp");
                    services.AddDataProtection().PersistKeysToStackExchangeRedis(redis, "DataProtection-Keys");
                    if (Configuration.GetConnectionString("RedisConnectionUrl") != null)
                    {
                        services.AddStackExchangeRedisCache(options =>
                        {
                            options.Configuration = Configuration.GetConnectionString("RedisConnectionUrl");
                            options.InstanceName = "Nts";
                ...
                {
                    Console.WriteLine("Issue in Connecting Redis Cache " + ex.Message);
                }
            ...
            var builder = services.AddControllersWithViews().AddViewLocalization
                (opt =>
                {
            ...
            if (HostEnvironment.IsDevelopment())
            {
                ...
                var bls = Path.GetFullPath(Path.Combine(path, "..", "Synergy.App.BLS"));
                var emirates = Path.GetFullPath(Path.Combine(path, "..", "Synergy.App.Emirates"));
                ...
                builder.AddRazorRuntimeCompilation(options =>
                {
                    ...
                    options.FileProviders.Add(new PhysicalFileProvider(bls));
                    options.FileProviders.Add(new PhysicalFileProvider(emirates));
                });
            }
#endif
            ...
            services.AddDNTCaptcha(options =>
               ...
               options.UseDistributedCacheStorageProvider() // -> It relies on the server and client's times. It's ideal for scalability, because it doesn't save anything in the server's memory.
      ...
      "css/jquery-splitter.css",
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapAreaControllerRoute("csc", "Csc", "Csc/{controller}/{action}/{id?}");
                endpoints.MapAreaControllerRoute("bls", "BLS", "BLS/{controller}/{action}/{id?}");
                endpoints.MapAreaControllerRoute("casemanagement", "CaseManagement", "CaseManagement/{controller}/{action}/{id?}");
            ...
            try
            {
                ...
                // please don't remove/comment below line it is used for auto create migration script 
