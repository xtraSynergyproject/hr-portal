...
$(document).ready(function () {
    selectedTemplateId = document.getElementById("templateId").getAttribute("value");
...
function configureNode(s) {
    //var url = '/cms/BusinessDiagram/ConfigBusinessDiagramNode?nodeId=' + id;
    //var win = GetMainWindow();
    //win.iframeOpenUrl = url;
    //win.OpenWindow({ Title: 'Configure Node', Width: 600, Height: 600 });
    //return false;

    //iframeOpenUrl = url;
    //OpenIframePopup(300, 600, 'Configure Node');
...
this.paper.on('element:contextmenu', (node, x, y) => {
    ...
    if (node.model.attributes.attrs.type == 'TEMPLATE') {
        ...
        var nodeData = JSON.stringify(node.model);
        console.log(node);
        $("#menulist").append("<li class='menu-option' onclick='onStepTaskDiagram(\"" + node.model.attributes.attrs.dataModel.ReferenceId + "\");'>Open Step Task Diagram</li>");
