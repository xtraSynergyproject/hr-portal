...
namespace CMS.UI.Web.Areas.CMS.Controllers
{
    ...
    public class TaskController : ApplicationController
    {
        ...
        public async Task<IActionResult> Index(string taskId, string assignTo, string teamId, string templateCode1, ReferenceTypeEnum referenceTypeCode, string referenceId, bool isPopUp = false, long? versionId = null, string udf1 = null, string udf2 = null, string udfValue1 = null, string udfValue2 = null)
        {
            ...
            if (isPopUp)
            {
                ViewBag.Layout = "~/Areas/Core/Views/Shared/_PopupLayoutTask.cshtml";
            }
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    try
                    {
                        ...
                        await hangfireScheduler.Enqueue<Synergy.App.Business.HangfireScheduler>(x => x.SendEmailUsingHangfire(result.Item.EmailUniqueId));
                    }
                    catch (Exception)
                    {
        ...
        {
            var result = await _taskBusiness.GetActiveListByUserId(_userContext.UserId);
            var j = Json(result.Where(x => x.TemplateCode == "CONFIRM_INDUCTION_DATE_TO_CANDIDATE" && (x.TaskStatusCode == "INPROGRESS" || x.TaskStatusCode == "OVERDUE")).OrderByDescending(x => x.StartDate));
        ...
        {
            return View("~/Areas/Core/Views/Cms/Task.cshtml");
        }
