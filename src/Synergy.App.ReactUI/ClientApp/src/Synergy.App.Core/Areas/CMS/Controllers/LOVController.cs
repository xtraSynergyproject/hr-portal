...
namespace CMS.UI.Web.Areas.CMS.Controllers
{
    ...
    public class LOVController : ApplicationController
    {
            ...
            if (page!=null) 
            {
                ...
                if (template!=null)
                {
                    if (template.TemplateType==TemplateTypeEnum.Custom) 
                    {
                        ...
                        var lov = await _LOVBusiness.GetSingleById(model.LOVTypeId);
                        if (lov.IsNotNull())
                        {
                            model.LOVTypeCode = lov.Code;
                        }
                    }
                }
        ...
        [HttpGet]
        public async Task<JsonResult> GetListOfValueList(string type,bool includePlaceHolder=false)
        {
        ...
        public async Task<JsonResult> GetAllListOfValue(string type)
        {
            ...
            {
                var parentexist = await _LOVBusiness.GetSingle(x => x.Code == type);
                if (parentexist.IsNotNull() && parentexist.ParentId.IsNotNullAndNotEmpty())
                {
                    var parent = await _LOVBusiness.GetSingle(x => x.Id == parentexist.ParentId);
                    data = data.Where(x => x.LOVType == parent.Code).ToList();
                }
            }
            return Json(data.OrderBy(x => x.SequenceOrder));
        }
        ...
        {
            var model = new List<LOVViewModel>();
            
            if (lovTypeId.IsNotNullAndNotEmpty())
            {
                var lov = await _LOVBusiness.GetSingleById(lovTypeId);
                model = await _LOVBusiness.GetList(x => x.LOVType == lov.Code, y => y.Parent);
                model.ForEach(x =>
                x.ParentName = x.ParentId.IsNotNullAndNotEmpty() ? _LOVBusiness.GetSingleById(x.ParentId).Result.Name : null);
            }
            else
            {
                model = await _LOVBusiness.GetList();
            }
            var dsResult = model.OrderBy(x => x.SequenceOrder);
            return Json(dsResult);
        }
        ...
        public async Task<IActionResult> ManageLOV(LOVViewModel model)
        {
            if (ModelState.IsValid)
            {
                var lov = await _LOVBusiness.GetSingle(x => x.Id == model.LOVType);
                if (lov.IsNotNull())
                {
                    model.LOVType = lov.Code;
                }
                if (model.DataAction == DataActionEnum.Create)
                {
                    var result = await _LOVBusiness.Create(model);
        ...
        public async Task<ActionResult> GetLOVIdNameList(string lovType,string excludeItem=null)
        {
            ...
            {
                list = list.Where(x => x.Code != excludeItem).ToList();
            }
