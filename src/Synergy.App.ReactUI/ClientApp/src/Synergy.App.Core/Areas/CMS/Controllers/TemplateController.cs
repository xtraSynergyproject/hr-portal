...
namespace CMS.UI.Web.Areas.CMS.Controllers
{
    ...
    public class TemplateController : Controller
    {
        ...
        private readonly IStringLocalizer<Synergy.App.WebUtility.Resources.Areas.Controllers.SharedResource> _localizer;
        ...
        IFileBusiness fileBusiness, IConfiguration configuration,
        ICompanySettingBusiness companySettings,
        ...
        IWebHelper webApi, IRecTaskTemplateBusiness recTaskTemplateBusiness
            ...
            , IStringLocalizer<Synergy.App.WebUtility.Resources.Areas.Controllers.SharedResource> localizer
        ...
        public async Task<JsonResult> GetTemplateListByPortal(string portalId)
        {
            var data = await _templateBusiness.GetList(x => x.PortalId == portalId);
            return Json(data);
        }
        ...
        public async Task<IActionResult> ManageServiceUdfs(string id, string templateId, LayoutModeEnum lo = LayoutModeEnum.Main, string cbm = null)
        {
            var model = new ServiceTemplateViewModel();
            ...
            ViewBag.LayoutMode = Convert.ToString(lo);
            ViewBag.SynergyTableMapId = await GetTableMetadataNameByTemplateId(templateId);
            return View("_ManageServiceUdfs", model);
        }
        ...
        public async Task<ActionResult> ReadFormTableProperties(/*[DataSourceRequest] DataSourceRequest request,*/ string templateId, string indexPageTemplateId)
        {
            ...
            foreach (var column in columnMetaData.Where(x => x.IsHiddenColumn == false))
            {
                ...
                {
                    var existingColumn = pageColumns.FirstOrDefault(x => x.ColumnName == column.Name);
        ...
        [HttpPost]
        public async Task<ActionResult> ManageServiceUdfs(ServiceTemplateViewModel model)
        {
            if (ModelState.IsValid)
            {
                model.Json = _webApi.RemoveHost(model.Json);
                ...
                {
                    var existmodel = await _serviceTemplateBusiness.GetSingleById(model.Id);
                    existmodel.Json = model.Json;
                    existmodel.FormType = model.FormType;
                    var Taskresult = await _serviceTemplateBusiness.Edit(existmodel);
                    if (Taskresult.IsSuccess)
                    {
                        return Json(new { success = true, templateId = Taskresult.Item.TemplateId });
                    }
        ...
        {
            var model = new ResourceLanguageViewModel { TemplateId = templateId, DataAction = DataActionEnum.Create };
        ...
        public async Task<ActionResult> GetTemplateLanguage([DataSourceRequest] DataSourceRequest request, string templateId)
        {
            ...
            jsonList = jsonList.OrderBy(x => x.Code).ToList();
            jsonList.ForEach(x =>
            {
                x.TemplateId = templateId;
                x.TemplateType = template.TemplateType;
                x.GroupCode = ResourceLanguageGroupCodeEnum.Udf;
                x.DataAction = DataActionEnum.Create;
            ...
            var other = actualList.Where(x => !result.Any(y => y.Code == x.Code));
                ...
                if (typeObj.IsNotNull())
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            JToken key = jcomp["label"];
                            JToken colId = jcomp["columnMetadataId"];
                            if (key != null && colId != null && !list.Any(x => x.Code == key.Value<string>()))
                            {
                                model.English = model.Code = key.IsNotNull() ? key.Value<string>() : "";
                                list.Add(model);
                            }
        ...
        public async Task<IEnumerable<object>> GetLanguage(string templateId)
        {
            ...
            foreach (var jsonItem in jsonList)
            {
                var item = actualList.FirstOrDefault(x => x.Code == jsonItem.Code);
                if (item.IsNotNull())
                {
                    result.Add(item);
                }
        ...
        public async Task<IActionResult> DownloadSampleFile(string TemplateId)
        {

            var ms = await _tableMetadataBusiness.GetExcelForTemplateUdf(TemplateId);
            var report = string.Concat("SampleTemplate", ".xlsx");
            return File(ms, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", report);
        ...
        public async Task<IActionResult> DownloadNoteTemplateDataFile(string TemplateId)
        {

            var ms = await _tableMetadataBusiness.GetExcelForNoteTemplateData(TemplateId);
            var report = string.Concat("TemplateData", ".xlsx");
            return File(ms, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", report);

        }
        public async Task<IActionResult> UploadTemplate(string TemplateId)
        {
        ...
        public async Task<ActionResult> ImportTemplateData(IList<IFormFile> file, string templateId)
        {
            ...
            if (file != null)
            {
                ...
                if (result.Count >= 0)
                {
                    ...
                    return Json(new { success = true, error = result,filename= fileName });
                }
                else
                {
        ...
        public async Task<List<string>> UploadFormDetails(string physicalPath, string templateId)
        {
                    ...
                    using (TextFieldParser parser = new TextFieldParser(physicalPath))
                    {
                        ...
                        while (!parser.EndOfData)
                        {
                            ...
                            foreach (var col in columns.Where(x => x.IsUdfColumn == true))
                            {
                                if (col.IsForeignKey)
                                {
                                    var fkeyId = await GetForeignKeyId(col, fields[i]);
                                    ((IDictionary<String, Object>)exo).Add(col.Name, fkeyId);
                                }
                                else
                                {
                            ...
                            var formModel = new FormTemplateViewModel()
                            {
                                Json = Json,
                            ...
                            var formResult = await _cmsBusiness.ManageForm(formModel);
                            ...
                            if (!formResult.IsSuccess)
                            {
                                errorList.Add(formResult.Message);
                                errorList.Add("Row " + j + " does not get inserted \n");
                            }
                        }
                        return errorList;
        ...
        {
            var data = await _templateBusiness.GetForeignKeyId(col, val);
            return data;
        }

        public async Task<List<string>> UploadNoteDetails(string physicalPath, string templateId)
        {
            var _cmsBusiness = _serviceProvider.GetService<ICmsBusiness>();
                ...
                if (System.IO.File.Exists(physicalPath))
                {
                    ...
                    using (TextFieldParser parser = new TextFieldParser(physicalPath))
                    {
                        ...
                        while (!parser.EndOfData)
                        {
                            ...
                            noteModel.NoteSubject = fields[0];
                            noteModel.NoteDescription = fields[1];
        ...
        public async Task<IActionResult> FetchTemplatesListByTemplateType(string type)
        {
            try
            {
                List<TemplateViewModel> list = new List<TemplateViewModel>();
                //var eldbUrl = ApplicationConstant.AppSettings.WebApiUrl(_configuration);
                var eldbUrlData = await _companySettings.GetSingle(x => x.Code == "WEB_DEV_API_URL");
                var eldbUrl = eldbUrlData.Value;
                var settings = new Nest.ConnectionSettings(new Uri(eldbUrl));
                ...
                {
                    var url = eldbUrl + "common/query/GetTemplatesList?templateType=" + type;
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (data.IsNotNull()) 
                        {
                            foreach (var x in data)
                            {
                                ...
                                var result = JsonConvert.DeserializeObject<TemplateViewModel>(str);
                                list.Add(result);
                            }
                        }
                ...
                var _cmsBusiness = _serviceProvider.GetService<ICmsBusiness>();
                TemplateTypeEnum templateTypeName = (TemplateTypeEnum)Enum.Parse(typeof(TemplateTypeEnum), type, true);
                int i = (int)(object)(templateTypeName);

                List<TemplateViewModel> presentTempList = await _cmsBusiness.GetTemplateListByTemplateType(i);

                List<TemplateViewModel> newTempList = new List<TemplateViewModel>();
                //var t = list.Where(x => presentTempList.All(y => x.Code != y.Code)).ToList();
                newTempList = list.Where(x => !presentTempList.Any(y => x.Code == y.Code)).ToList();
                //var newTempList2 = list.Where(x => presentTempList.Any(y => x.Code == y.Code)).ToList();
                
                //newTempList = list.Intersect(presentTempList).ToList();
                return Json(newTempList);
            }
            catch (Exception ex)
            {
                return Json(ex);
        ...
        public async Task<IActionResult> FetchTemplateDataById(string ids)
        {
            ...
            try
            {
                ...
                foreach(var id in idList)
                {
                    ...
                    using (var httpClient = new HttpClient(handler))
                    {
                        var url = eldbUrl + "common/query/GetTemplateData?id=" + id;
                        var address = new Uri(url);
                        var response = await httpClient.GetAsync(address);
                        ...
                        {
                            var content = await response.Content.ReadAsStringAsync();
                            var x = JsonConvert.DeserializeObject<CopyTemplateViewModel>(content);
                            res = await _cmsBusiness.CopyDevTemplates(x);
                            if (!res)
                            {
                                return Json(new { success = false, id = id, name = x.Template.DisplayName });
                            }
                        }
                    }
                }
        ...
        public async Task<IActionResult> CopyTemplate(string id)
        {
            ...
            {
                var result = await _cmsBusiness.GetStepTaskTemplateListData(id);
                ViewBag.StepTaskData = result;
            }
        ...
        [HttpPost]
        public async Task<IActionResult> CopyTemplateData(TemplateViewModel model)
        {
            var _cmsBusiness = _serviceProvider.GetService<ICmsBusiness>();
            var oldTempId = model.Id;
            model.Id = null;
            if (model.TemplateType == TemplateTypeEnum.Note || model.TemplateType == TemplateTypeEnum.Service)
            {
                var isExists = await _templateBusiness.GetSingle(x => x.DisplayName == model.DisplayName);
                if (isExists.IsNotNull())
                {
                    return Json(new { success = false, error = "Display Name Already Exists. Please enter unique display name" });
                }
            }
            if (ModelState.IsValid)
            {
                ...
                if (result.IsSuccess)
                {
                    var newTemplateId = result.Item.Id;
                    if (model.TemplateType == TemplateTypeEnum.Service)
                    {
                        var res = await _cmsBusiness.CopyService(oldTempId, newTemplateId, model);

                    }
                    else if (model.TemplateType == TemplateTypeEnum.Note)
                    {
                        var res = await _cmsBusiness.CopyNote(oldTempId, newTemplateId);

                    }
                    else if (model.TemplateType == TemplateTypeEnum.Task)
                    {
                        var res = await _cmsBusiness.CopyTask(oldTempId, newTemplateId);
                    }
                    else if (model.TemplateType == TemplateTypeEnum.Page)
                    {
                        var res = await _cmsBusiness.CopyPage(oldTempId, newTemplateId);
                    }
                    else if (model.TemplateType == TemplateTypeEnum.Form)
                    {
                        var res = await _cmsBusiness.CopyForm(oldTempId, newTemplateId);

                    }
                    else if(model.TemplateType == TemplateTypeEnum.Custom)
                    {
                        var res = await _cmsBusiness.CopyCustomTemplate(oldTempId, newTemplateId);
                    }
                    return Json(new { success = true });
