...
namespace CMS.UI.Web.Areas.PortalAdmin.Controllers
{
...

    {
       ...
       /* public async Task<IActionResult> Edit(string Id)
        ...
        public async Task<IActionResult> GetTeamWithUsersByPortalId(string portalId,string userId)
        {
            var teamlist = await _teamuserBusiness.GetList(x => x.UserId == userId && x.PortalId == portalId);
            string[] teams = teamlist.Select(x => x.TeamId).ToArray();
            string tIds = string.Join(",", teams);

            var data = await _portalBusiness.GetTeamWithUsersByPortalId(portalId, tIds);

            return Json(data);
        }

        public async Task<ActionResult> GetTeamMemberListWithTeamOwner(string teamId)
        {
            var data = await _teamBusiness.GetTeamMemberList(teamId);
            var teamowner = data.Where(x => x.TeamOwnerId == _userContext.UserId).FirstOrDefault();
            if (teamowner.IsNotNull())
            {
