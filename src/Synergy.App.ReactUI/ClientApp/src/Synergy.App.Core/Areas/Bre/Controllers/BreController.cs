...
namespace CMS.UI.Web.Controllers
{
    ...
    public class BreController : ApplicationController
    {
        ...
        public async Task<IActionResult> BreResultViewDetails(string businessRuleNodeId, string templateId, string parentId, string ruleId, bool? isBusinessDiagram, BusinessLogicExecutionTypeEnum? nodeType)
        {
            ...
            {
                if (businessRuleNodeId.Contains("PRE_LOAD"))
                {
                    businessRuleNodeId = businessRuleNodeId.Replace("PRE_LOAD_","").Trim();
                }
        ...
        {
            var result = await _ruleBusiness.ScanFolderNew(id, type, parentId, templateType, namespaces, methodName,null);
            return Json(result);
        }

        public async Task<object> GetFolderStructureFancyTree(string id, string type, string parentId, string templateType, string namespaces, string methodName, BusinessLogicExecutionTypeEnum? nodeType)
        {
            var result = await _ruleBusiness.ScanFolderNew(id, type, parentId, templateType, namespaces, methodName,nodeType);            
            var newList = new List<FileExplorerViewModel>();
        ...
        public async Task<IActionResult> GetUdfValues(string templateId)
        {
            var template = await _templateBusiness.GetSingleById(templateId);           
            var tableMetadata = await _tableMetadataBusiness.GetSingleById(template.UdfTableMetadataId);
            var result  = await _columnMetadataBusiness.GetViewableColumnMetadataList(tableMetadata.Schema, tableMetadata.Name, true);
            var data = new List<IdNameViewModel>();
            data= result.Select(x=>new IdNameViewModel()
            {
                Id=x.Id,
                Name=x.Alias,
                Code=x.Name
        ...
        public async Task<JsonResult> GetBusinessRuleModelListForStepTaskAssigneeLogicNew(string id, string nodeId, string stepTaskComponentId)
        {
