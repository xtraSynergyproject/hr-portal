...
namespace CMS.Business
{
    public class NotificationTemplateBusiness : BusinessBase<NotificationTemplateViewModel, NotificationTemplate>, INotificationTemplateBusiness
    {
        ...
        public async override Task<CommandResult<NotificationTemplateViewModel>> Create(NotificationTemplateViewModel model)
        {
            //var body = model.Body;
        ...
        public async override Task<CommandResult<NotificationTemplateViewModel>> Edit(NotificationTemplateViewModel model)
        {
            model.Body = HttpUtility.HtmlDecode(model.Body);
            var validateName = await IsNameExists(model);
            if (!validateName.IsSuccess)
