...
namespace CMS.UI.Web
{
    public class AccountController : ApplicationController
    {
        ...
        {
            var returnUrl = model.ReturnUrl ?? Url.Content("~/");
            var url = "https://api.belsmartcity.in/api/sc/signin";
            ...
            var response = await Helper.HttpRequest(url, HttpVerb.Post, loginDataJson);
            var token = JToken.Parse(response);
            var login = token.SelectToken("login");

            if (Convert.ToString(login) != "SUCCESS")
            {
                var createUserUrl = "https://api.belsmartcity.in/api/sc/createuser";
