...
namespace CMS.UI.Web.Areas.Pms.Controllers
{
    ...
    public class PerformanceController : ApplicationController
    {
        ...
        public async Task<ActionResult> GetPerformanceDetails(string MasterId = null, string deptId = null, string userId = null)
        {
            var dmaster = await _pmtBusiness.GetPDMDetails(MasterId);
            if (dmaster.IsNotNull())
            {
                var list = await _pmtBusiness.GetPerformanceDocumentDetailsData(dmaster.UdfNoteId);
