...
namespace CMS.Business
{
    public class BatchBusiness : BusinessBase<BatchViewModel, Batch>, IBatchBusiness
    {
        ...
        public async override Task<CommandResult<BatchViewModel>> Create(BatchViewModel model)
        {
        ...
        public async override Task<CommandResult<BatchViewModel>> Edit(BatchViewModel model)
        {
            var data = _autoMapper.Map<BatchViewModel>(model);
            var validateName = await IsNameExists(data);
            if (!validateName.IsSuccess)
